...
print_fio_header(){
  printf ' '; pad '块大小' 10; printf ' '; pad 'QD' 6; printf ' '; pad '随机读取' 18; printf ' '; pad '读 IOPS' 14; printf ' '; pad '随机写入' 18; printf ' '; pad '写 IOPS' 14; printf '\n'
}
print_fio_row(){
  local bs="$1" qd="$2" rb="$3" ri="$4" wb="$5" wi="$6"
  printf ' '; pad "$bs" 10; printf ' '; pad "$qd" 6; printf ' '; pad "${rb} MB/s" 18; printf ' '; pad "$ri" 14; printf ' '; pad "${wb} MB/s" 18; printf ' '; pad "$wi" 14; printf '\n'
}
print_fio(){
  rule
  ...
  for bs in 4k 64k 512k 1m; do
    ...
    if (( rc == 0 )); then
      ...
      print_fio_row "$bs" "$qd" "$rb" "$ri" "$wb" "$wi"
      pass=$((pass+1))
    else
      printf ' '; pad "$bs" 10; printf ' '; pad "$(fio_qd "$bs")" 6; printf ' 已停止（超时/不支持）\n'
      break
    fi
  done
  assess_fio
...
run_three_carrier_route(){
  ...
  if ! command -v timeout >/dev/null 2>&1; then ROUTE_VERDICT="环境不支持"; return 0; fi
  set +e
  timeout "${ROUTE_TIMEOUT_SEC}s" python3 - "$ROUTE_TSV" <<'PY'
import socket,sys
out=sys.argv[1]
targets=[('北京电信','219.141.140.10'),('北京联通','202.106.195.68'),('北京移动','221.179.155.161'),('上海电信','202.96.209.133'),('上海联通','210.22.97.1'),('上海移动','211.136.112.200'),('广州电信','58.60.188.222'),('广州联通','210.21.196.6'),('广州移动','120.196.165.24'),('成都电信','61.139.2.69'),('成都联通','119.6.6.6'),('成都移动','211.137.96.205')]
def classify(ip):
...
for name,dst in targets:
    ...
    try:
        ...
        for ttl in range(1,13):
            ...
            try: _,addr=recv.recvfrom(2048); hop=addr[0]
...
print_route_header(){
  printf ' '; pad '地区/运营商' 16; printf ' '; pad '回程线路' 18; printf ' '; pad '等级' 10; printf '\n'
}
print_route_row(){
  printf ' '; pad "$1" 16; printf ' '; pad "$2" 18; printf ' '; pad "$3" 10; printf '\n'
}
print_three_carrier_route(){
  ...
  run_three_carrier_route
  local n good ct_good cu_good cm_good
  ...
  print_route_header
  while IFS=$'\t' read -r name route grade; do print_route_row "$name" "$route" "$grade"; done <"$ROUTE_TSV"
  good="$(awk -F '\t' '$3=="优质"{n++} END{print n+0}' "$ROUTE_TSV")"
  ct_good="$(awk -F '\t' '$1~/电信$/ && $3=="优质"{n++} END{print n+0}' "$ROUTE_TSV")"
  cu_good="$(awk -F '\t' '$1~/联通$/ && $3=="优质"{n++} END{print n+0}' "$ROUTE_TSV")"
  cm_good="$(awk -F '\t' '$1~/移动$/ && $3=="优质"{n++} END{print n+0}' "$ROUTE_TSV")"
  if (( n >= 9 )); then ROUTE_VERDICT="有效 ${n}/12，优质 ${good}/${n}"
  else ROUTE_VERDICT="部分有效 ${n}/12，优质 ${good}/${n}"; fi
  printf ' 电信优质回程       : %s / 4\n' "$ct_good"
  printf ' 联通优质回程       : %s / 4\n' "$cu_good"
  printf ' 移动优质回程       : %s / 4\n' "$cm_good"
  printf ' 三网回程证据       : %s\n' "$ROUTE_VERDICT"
