...
$APP $VERSION
Usage:
  p07-bench.sh              Run the complete one-shot VPS acceptance test
  p07-bench.sh --demo       Render a no-impact demo result
  p07-bench.sh --self-test  Run internal logic tests only
  p07-bench.sh --version

Environment:
  P07_BENCH_IO_MIB=256      Per-round disk write size; 3 rounds are run
  P07_BENCH_DIR=/var/tmp    Real filesystem directory used for I/O test
EOH
      exit 0 ;;
...
if [[ -t 1 && -z "${NO_COLOR:-}" ]]; then
  ...
  YELLOW=$'\033[0;33m'; BLUE=$'\033[0;36m'; MAGENTA=$'\033[0;35m'; GRAY=$'\033[0;90m'
else
...
cleanup(){
  if [[ $SPEEDTEST_TEMP -eq 1 && -n "$SPEEDTEST_BIN" ]]; then rm -rf "$(dirname "$SPEEDTEST_BIN")" 2>/dev/null || true; fi
...
get_os(){
  if [[ -r /etc/os-release ]]; then awk -F= '/^PRETTY_NAME=/{gsub(/^"|"$/,"",$2);print $2}' /etc/os-release
  elif [[ -r /etc/redhat-release ]]; then cat /etc/redhat-release
  else uname -s; fi
}
get_arch(){ uname -m 2>/dev/null || printf UNKNOWN; }
get_kernel(){ uname -r 2>/dev/null || printf UNKNOWN; }
get_cpu(){
  if command -v lscpu >/dev/null 2>&1; then lscpu | awk -F: '/Model name/{gsub(/^[ \t]+/,"",$2);print $2;exit}'
  else awk -F: '/model name/{gsub(/^[ \t]+/,"",$2);print $2;exit}' /proc/cpuinfo 2>/dev/null; fi
}
get_cores(){ getconf _NPROCESSORS_ONLN 2>/dev/null || nproc 2>/dev/null || printf '?'; }
get_freq(){
  local v=""
  if command -v lscpu >/dev/null 2>&1; then v="$(lscpu | awk -F: '/CPU max MHz/{gsub(/^[ \t]+/,"",$2);printf "%.0f MHz",$2;exit}')"; fi
...
get_rootfs(){ df -hPT / 2>/dev/null | awk 'NR==2{print $2"|"$3"|"$4"|"$5"|"$6}' || printf 'UNKNOWN|UNKNOWN|UNKNOWN|UNKNOWN|UNKNOWN'; }
...
import sqlite3,sys
try:
 c=sqlite3.connect(f'file:{sys.argv[1]}?mode=ro',uri=True); n=c.execute('select count(*) from site').fetchone()[0]; c.close(); print(f'Installed|{n}')
except Exception: print('Installed|UNKNOWN')
PYCP
  elif command -v clpctl >/dev/null 2>&1 || [[ -d /home/clp/htdocs/app ]]; then printf 'Installed|UNKNOWN'
  else printf 'Not detected|0'; fi
}
get_ipmeta(){
  ...
  local raw="$(curl -4fsS --connect-timeout 4 --max-time 8 https://ipinfo.io/json 2>/dev/null || true)"
  if [[ -n "$raw" && -x "$(command -v python3 2>/dev/null || true)" ]]; then
...
collect_system(){
  ...
  IFS='|' read -r ROOT_FS ROOT_TOTAL ROOT_USED ROOT_FREE ROOT_PCT <<<"$(get_rootfs)"
...
print_system(){
  ...
  printf ' CPU Cache          : %s\n' "${CACHE:-UNKNOWN}"
  printf ' AES-NI             : %s\n' "$AES"
  printf ' Nested Virt Flag   : %s\n' "$NESTED"
  printf ' Root Disk          : %s%s%s  (%s used, %s free, %s)\n' "$YELLOW" "$ROOT_TOTAL" "$RESET" "$ROOT_USED" "$ROOT_FREE" "$ROOT_FS"
  printf ' Total Mem          : %s%s%s  (%s used)\n' "$YELLOW" "$RAM_TOTAL" "$RESET" "$RAM_USED"
  ...
  if [[ "$CLOUDPANEL_STATE" == Installed ]]; then printf ' CloudPanel         : %sInstalled%s · %s site(s)\n' "$GREEN" "$RESET" "$CLOUDPANEL_SITES"; else printf ' CloudPanel         : %sNot detected%s\n' "$GRAY" "$RESET"; fi
...
rate_to_mb(){
  ...
  awk -v v="$value" -v u="$unit" 'BEGIN{if(v!~/^[0-9.]+$/){print 0;exit} if(u=="GB/s")printf "%.2f",v*1000; else if(u=="MB/s")printf "%.2f",v; else if(u=="kB/s")printf "%.2f",v/1000; else if(u=="B/s")printf "%.2f",v/1000000; else print 0}'
}
run_dd_once(){
  local file="$1" out rc raw
  if (( DEMO )); then case "$2" in 1) printf '577 MB/s';; 2) printf '581 MB/s';; *) printf '565 MB/s';; esac; return 0; fi
  out="$(LANG=C dd if=/dev/zero of="$file" bs=1M count="$IO_MIB" oflag=direct conv=fdatasync 2>&1)"; rc=$?
  if (( rc != 0 )); then out="$(LANG=C dd if=/dev/zero of="$file" bs=1M count="$IO_MIB" conv=fdatasync 2>&1)"; rc=$?; fi
...
print_io(){
  ...
  if ! [[ "$IO_MIB" =~ ^[0-9]+$ ]] || (( IO_MIB < 64 || IO_MIB > 1024 )); then IO_STATE="SKIPPED"; printf ' %-18s: invalid P07_BENCH_IO_MIB (64-1024 required)\n' 'I/O Test'; return; fi
  if (( ! DEMO )); then
    [[ -d "$BENCH_DIR" && -w "$BENCH_DIR" ]] || { IO_STATE="SKIPPED"; printf ' %-18s: directory not writable\n' 'I/O Test'; return; }
    local fs free_mib
    fs="$(df -PT "$BENCH_DIR" 2>/dev/null | awk 'NR==2{print $2}')"
    if [[ "$fs" == tmpfs || "$fs" == devtmpfs ]]; then IO_STATE="SKIPPED"; printf ' %-18s: blocked on %s (not real disk)\n' 'I/O Test' "$fs"; return; fi
    free_mib="$(df -Pm "$BENCH_DIR" 2>/dev/null | awk 'NR==2{print $4}')"
    [[ "$free_mib" =~ ^[0-9]+$ ]] && (( free_mib >= IO_MIB + 256 )) || { IO_STATE="SKIPPED"; printf ' %-18s: insufficient free space\n' 'I/O Test'; return; }
  fi
  local i raw mb sum="0"
  for i in 1 2 3; do
    raw="$(run_dd_once "$BENCH_DIR/.p07-bench-$$-$i.dat" "$i" || true)"
    if [[ "$raw" == FAIL || -z "$raw" ]]; then IO_STATE="FAIL"; printf ' I/O #%-13s: %sFAIL%s\n' "$i" "$RED" "$RESET"; continue; fi
    mb="$(rate_to_mb "$raw")"; IO_RATES+=("$mb"); sum="$(awk -v a="$sum" -v b="$mb" 'BEGIN{printf "%.2f",a+b}')"
    printf ' I/O #%-13s: %s%s%s\n' "$i" "$GREEN" "$raw" "$RESET"
  done
  if ((${#IO_RATES[@]})); then IO_AVG_MB="$(awk -v s="$sum" -v n="${#IO_RATES[@]}" 'BEGIN{printf "%.2f",s/n}')"; printf ' Average            : %s%s MB/s%s\n' "$GREEN" "$IO_AVG_MB" "$RESET"; else IO_STATE="FAIL"; fi
}
...
prepare_speedtest(){
  ...
  local machine pkg sha url d got
  machine="$(uname -m)"
  case "$machine" in
    x86_64|amd64) pkg=x86_64; sha="$OOKLA_X86_64_SHA256" ;;
    aarch64|arm64) pkg=aarch64; sha="$OOKLA_AARCH64_SHA256" ;;
    *) return 1 ;;
  esac
  d="$TMP_DIR/ookla"; mkdir -p "$d"
  url="https://install.speedtest.net/app/cli/ookla-speedtest-${OOKLA_VERSION}-linux-${pkg}.tgz"
  curl -fL --retry 2 --connect-timeout 8 --max-time 60 "$url" -o "$d/speedtest.tgz" >/dev/null 2>&1 || return 1
  got="$(sha256sum "$d/speedtest.tgz" | awk '{print $1}')"
  [[ "$got" == "$sha" ]] || { printf '%sSpeedtest package checksum mismatch; network benchmark skipped.%s\n' "$RED" "$RESET"; return 1; }
  tar -xzf "$d/speedtest.tgz" -C "$d" speedtest >/dev/null 2>&1 || return 1
  chmod 0755 "$d/speedtest"; SPEEDTEST_BIN="$d/speedtest"; SPEEDTEST_TEMP=1
}
network_reason(){
  local rc="$1" text="$2"
...
speed_node(){
  ...
  elif [[ -z "$SPEEDTEST_BIN" ]]; then state=UNAVAILABLE; reason=SPEEDTEST_UNAVAILABLE; up='-'; down='-'; lat='-'
  elif ! command -v timeout >/dev/null 2>&1; then state=UNAVAILABLE; reason=TIMEOUT_COMMAND_MISSING; up='-'; down='-'; lat='-'
  else
...
import json,sys
try:
 d=json.loads(sys.argv[1]); print(f"{d['upload']['bandwidth']*8/1e6:.2f}|{d['download']['bandwidth']*8/1e6:.2f}|{d['ping']['latency']:.2f}")
except Exception: pass
PY
)"
    fi
    if [[ -n "$parsed" ]]; then IFS='|' read -r up down lat <<<"$parsed"; state=PASS; reason=NONE
    else state=FAIL; reason="$(network_reason "${rc:-1}" "$raw")"; up='-'; down='-'; lat='-'; fi
  fi
...
http_probe(){
  local url="$1" rc
  if (( DEMO )); then [[ "$url" == *baidu* || "$url" == *qq.com* || "$url" == *taobao* || "$url" == *189.cn* || "$url" == *10010* || "$url" == *10086* ]] && return 1 || return 0; fi
  curl -LsS -o /dev/null --connect-timeout 4 --max-time 8 "$url" >/dev/null 2>&1; rc=$?
  return "$rc"
}
count_speed(){
...
china_assessment(){
  ...
  if (( cn_speed_pass >= 1 && mainland_http >= 4 )); then CHINA_VERDICT="NORMAL"; CHINA_RECOMMENDATION="IP looks usable from current outbound signals"
  elif (( cn_speed_fail >= 2 && overseas_speed_pass >= 5 && mainland_fail >= 3 && overseas_http >= 2 )); then CHINA_VERDICT="HIGH_RISK"; CHINA_RECOMMENDATION="CHANGE IP BEFORE MIGRATION"
  elif (( cn_speed_fail >= 2 && overseas_speed_pass >= 3 )) || (( mainland_fail >= 2 && overseas_http >= 2 )); then CHINA_VERDICT="RISK"; CHINA_RECOMMENDATION="VERIFY FROM MAINLAND BEFORE MIGRATION"
  fi
  rule
...
write_reports(){
  ...
  ts="$(date +%Y%m%d_%H%M%S)"; REPORT_TXT="$REPORT_DIR/p07-bench_${ts}.txt"; REPORT_JSON="$REPORT_DIR/p07-bench_${ts}.json"
  ...
  if command -v python3 >/dev/null 2>&1; then
    python3 - "$REPORT_JSON" "$VERSION" "$OS" "$CPU" "$CORES" "$RAM_TOTAL" "$ROOT_TOTAL" "$IPV4" "$ORG" "$CITY" "$COUNTRY" "$IO_STATE" "$IO_AVG_MB" "$CHINA_VERDICT" "$CHINA_RECOMMENDATION" "$elapsed" "$NETWORK_TSV" <<'PY' 2>/dev/null || true
import json,sys,datetime
(path,version,osname,cpu,cores,ram,disk,ipv4,org,city,country,io_state,io_avg,verdict,reco,elapsed,tsv)=sys.argv[1:]
nodes=[]
try:
...
payload={'schema_version':1,'module':'p07-bench','version':version,'timestamp':datetime.datetime.now(datetime.timezone.utc).astimezone().isoformat(),'system':{'os':osname,'cpu':cpu,'cores':cores,'ram':ram,'root_disk':disk,'ipv4':ipv4,'organization':org,'city':city,'country':country},'io':{'state':io_state,'average_mb_s':io_avg},'network_nodes':nodes,'china_ip_assessment':{'verdict':verdict,'recommendation':reco,'evidence_scope':'VPS_OUTBOUND_ONLY','mainland_origin_probe':'NOT_IMPLEMENTED'},'elapsed_seconds':int(elapsed)}
...
self_test(){
  ...
  [[ ${#OOKLA_X86_64_SHA256} -eq 64 && ${#OOKLA_AARCH64_SHA256} -eq 64 ]] || f=1
  local old_demo="$DEMO"; DEMO=1
  : >"$NETWORK_TSV"; speed_node 5396 'Suzhou, CN' >/dev/null; speed_node 59387 'Ningbo, CN' >/dev/null
