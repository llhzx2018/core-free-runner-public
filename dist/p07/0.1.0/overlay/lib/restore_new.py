...
CLOUDPANEL_DB_USER_RE = re.compile(r"^[A-Za-z0-9]{1,32}$")
...
def restore_new_site(package_dir: Path, target_root: Path, clpctl: str, confirm: str) -> dict:
    ...
    database_identity_remapped = False
    remapped_database: tuple[str, str, str] | None = None
    application_config_mode = "SOURCE_DATABASE_IDENTITY_PRESERVED"
    application_config_file: str | None = None
    ...
    try:
        ...
        for index, item in enumerate(mysql_entries, 1):
            ...
            source_username, source_password = restore_apply.database_credentials(row, database)
            username, password = source_username, source_password
            ...
            try:
                ...
                if CLOUDPANEL_DB_USER_RE.fullmatch(source_username):
                    raise
                if len(mysql_entries) != 1:
                    raise NewSiteRestoreError(
                        "CloudPanel rejected a legacy database user and automatic remap "
                        "requires a single application database"
                    )
                # Real CloudPanel rejects some legacy user identities (for example
                # usernames with underscores). Retry only inside this newly-created,
                # transaction-owned target with a strict alphanumeric identity.
                _, username, password = site_lifecycle.derive_database_identity(
                    domain, str(manifest.get("backup_id", "")), index
                )
                try:
                    restore_apply.run_clpctl(clpctl, [
                        "db:add",
                    ...
                    raise NewSiteRestoreError(
                        "CloudPanel database creation failed after compatibility retry"
                    ) from exc
                database_identity_remapped = True
                remapped_database = (database, username, password)
        ...
        if remapped_database is not None:
            failure_stage = "APPLICATION_DATABASE_CONFIG_REMAP"
            database, username, password = remapped_database
            try:
                application_config_mode, config_path = app_config.rewrite_application_database_config(
                    final_site, domain, domain, database, username, password
                )
            except app_config.AppConfigError as exc:
                raise NewSiteRestoreError(
                    "target database identity changed but application config could not be remapped"
                ) from exc
            if config_path is not None:
                application_config_file = config_path.relative_to(final_site).as_posix()
                owner = final_site.stat()
                os.chown(config_path, owner.st_uid, owner.st_gid, follow_symlinks=False)
            steps.append("database_identity_remapped_and_application_config_updated")
        ...
        return {
            ...
            "database_identity_remapped": database_identity_remapped,
            "application_config_mode": application_config_mode,
