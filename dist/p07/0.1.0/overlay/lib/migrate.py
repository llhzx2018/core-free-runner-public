...
def new_site_migrate(
    ...
    try:
        ...
        if defer_runtime:
            # Cron/PM2 are intentionally absent on the staged target. They must not
            # turn a correct pre-cutover data/site stage into a false migration FAIL.
            deferred_fields = {"cron.entry_count", "pm2.processes"}
            for item in cross_site.get("checks", []):
                if isinstance(item, dict) and item.get("field") in deferred_fields:
                    item["status"] = "DEFERRED"
            cross_site["failures"] = [
                field for field in cross_site.get("failures", []) if field not in deferred_fields
            ]
            cross_site["unknowns"] = [
                field for field in cross_site.get("unknowns", []) if field not in deferred_fields
            ]
            cross_site["deferred"] = sorted(deferred_fields)
            cross_site["status"] = (
                "FAIL" if cross_site["failures"]
                else ("UNKNOWN" if cross_site["unknowns"] else "PASS")
            )
    ...
    if defer_runtime:
        probe = {
            ...
            "failures": [],
            "dns_changed": False,
            "method": "DEFERRED_UNTIL_RUNTIME_ACTIVATION",
        }
    else:
        try:
    ...
    result: dict[str, Any] = {
        ...
        "status": ("MIGRATION_STAGED" if defer_runtime and ready else ("TECHNICAL_CUTOVER_READY" if ready else "NEW_SITE_MIGRATION_NOT_READY")),
        "backup_id": backup_id,
        "domain": domain,
        ...
        "technical_cutover_ready": ready and not defer_runtime,
        "staged_for_cutover": ready and defer_runtime,
...
def main() -> int:
    ...
    new_site.add_argument("--defer-runtime", action="store_true")
    ...
    try:
        if args.command == "apply-sandbox":
            ...
            success = result["status"] in {"TECHNICAL_CUTOVER_READY", "MIGRATION_STAGED"}
