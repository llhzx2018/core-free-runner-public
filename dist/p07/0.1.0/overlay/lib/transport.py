...
def parse_remote_result(stdout: str, *, staged: bool = False) -> dict[str, Any]:
    try:
    ...
    expected = "MIGRATION_STAGED" if staged else "TECHNICAL_CUTOVER_READY"
    if payload.get("status") != expected:
        raise TransportError(f"target migration did not reach {expected}")
    ...
    if staged:
        if payload.get("staged_for_cutover") is not True or payload.get("runtime_deferred") is not True:
            raise TransportError("target staged migration result is inconsistent")
        if payload.get("technical_cutover_ready") is not False:
            raise TransportError("staged migration must not claim technical cutover readiness")
    elif payload.get("technical_cutover_ready") is not True:
...
def transfer_new_site(
    ...
    try:
        ...
        if defer_runtime:
            migrate_args.append("--defer-runtime")
        ...
        remote_result = parse_remote_result(proc.stdout, staged=defer_runtime)
    ...
    return {
        "schema": SCHEMA,
        "status": "MIGRATION_STAGED" if defer_runtime else "TECHNICAL_CUTOVER_READY",
        "backup_id": backup_id,
        "domain": domain,
        ...
        "technical_cutover_ready": not defer_runtime,
        "staged_for_cutover": defer_runtime,
