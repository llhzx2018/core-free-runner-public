...
def apply_runtime(
    ...
    manual_cron = plan["manual_cron"]
    if manual_cron:
        system_only = all(
            isinstance(item, dict)
            and item.get("reason") == "SYSTEM_CRON_REQUIRES_MANUAL_RECONCILIATION"
            and str(item.get("source", "")).startswith("/etc/cron.d/")
            for item in manual_cron
        )
        if not (defer_system_cron and system_only):
    ...
    try:
        ...
        return {
            ...
            "manual_cron_remaining": manual_cron if defer_system_cron else [],
            "system_cron_deferred": bool(manual_cron and defer_system_cron),
...
def main() -> int:
    ...
    apply.add_argument("--defer-system-cron", action="store_true")
    ...
    try:
        if args.command == "plan":
            ...
            result = apply_runtime(
                Path(args.package), Path(args.target_root), args.confirm,
                args.crontab, args.runuser, args.pm2, args.chown,
                args.defer_system_cron,
            )
