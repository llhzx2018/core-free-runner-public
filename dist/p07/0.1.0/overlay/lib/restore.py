...
def target_has_site(target_root: Path, domain: str, site_root: str) -> bool:
    final_site = target_path(target_root, site_root)
    if os.path.lexists(final_site):
        return True
    try:
...
def build_plan(package_dir: Path, target_root: Path) -> dict[str, Any]:
    ...
    target_exists = target_has_site(target_root, domain, site_root)
    blockers: list[str] = []
    warnings: list[str] = []
    gates: list[str] = []
    ...
    if target_exists:
        blockers.append("TARGET_SITE_ALREADY_EXISTS")
    ...
    return {
        ...
        "owner_overwrite_gate_required": False,
...
def blocked_plan_error_message(result: dict[str, Any]) -> str:
    blockers = result.get("blockers", [])
    if isinstance(blockers, list) and "TARGET_SITE_ALREADY_EXISTS" in blockers:
        return (
            "target site already exists; "
            "new-site restore refuses an existing target site"
        )
    return "restore plan is blocked"
...
def main() -> int:
    ...
    if result["status"] == "BLOCKED":
        print(f"ERROR: {blocked_plan_error_message(result)}", file=os.sys.stderr)
        return 6
    return 0
