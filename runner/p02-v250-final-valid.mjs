...
page.on('console', m => {
  if (m.type() === 'error' && !/409 \(Conflict\)/.test(m.text())) errors.push('console:' + m.text());
});

const box = async s => {
  const l = page.locator(s).first();
  return await l.isVisible() ? await l.boundingBox() : null;
};
const overlap = (a, b) => !!(
  a && b &&
...
const getSession = async () => page.evaluate(async () => {
  return await (await fetch('/api.php?action=session',{cache:'no-store'})).json();
});
...
if (!sess.site.auth) {
  ...
  await page.waitForFunction(async () => {
    const x = await (await fetch('/api.php?action=session',{cache:'no-store'})).json();
    return Boolean(x.site.auth);
  });
  sess = await getSession();
}

if (await page.evaluate(() => state.contentView) !== 'notebook') {
  throw new Error('fresh default workspace is not notebook');
}

const csrf = sess.csrf;
const post = async (action, body) => page.evaluate(async ({action,body,csrf}) => {
  return await (await fetch('/api.php?action=' + action, {
    method: 'POST',
...
const item = await post('content_save',{
  category_id:cid,
  title:'V2.5.0 UAUI Final',
  description:'摘要只在用户主动选择摘要模式时显示。',
  content:'# V2.5.0 UAUI Final\n\n正文阅读验证。\n\n## 第二节\n\n' + ('舒适阅读需要合适字号、行距、字重和内容宽度。 '.repeat(120)),
...
await page.evaluate(async cid0 => {
  await setContentView('list');
  await selectCategory(cid0);
}, cid);
...
if (await page.locator('[data-item-row="'+iid+'"] .content-meta').isVisible()) {
  throw new Error('minimal mode meta visible');
}
...
await page.evaluate(async ({cid0,iid0}) => {
  await setContentView('notebook');
  await selectCategory(cid0);
  await openReader(iid0,true);
}, {cid0:cid,iid0:iid});
...
const typography = await page.evaluate(() => {
  ...
  return {
    ...
    reader: parseFloat(getComputedStyle(r).fontSize),
    line: parseFloat(getComputedStyle(r).lineHeight),
    mode: document.body.dataset.v250ListMode
  };
});
if (typography.body < 14.5 || typography.reader < 16.8 || typography.line < 29 || typography.mode !== 'minimal') {
  throw new Error('typography ' + JSON.stringify(typography));
}
...
await page.mouse.move(hb.x+3,hb.y+100);
await page.mouse.down();
await page.mouse.move(hb.x+73,hb.y+100,{steps:4});
await page.mouse.up();
const resize = await page.evaluate(() => ({
  w:state.notebookPaneWidth,
  s:Number(localStorage.getItem('vftb-notebook-pane-width'))
}));
if (resize.w < w0+40 || Math.abs(resize.w-resize.s)>2) {
  throw new Error('resize ' + JSON.stringify({w0,resize}));
}

const visibleControls = () => page.locator(
  '#sidebarRailToggle:visible,#notebookPaneCollapse:visible,#notebookPaneRestore:visible'
).count();
if (await visibleControls() !== 2 || await page.locator('#menuBtn:visible').count()) {
  throw new Error('duplicate pane controls');
}
const before = await box('#notebookDetailScroll');

await page.locator('#notebookPaneCollapse').click();
await page.waitForFunction(() => state.notebookListCollapsed === true);
await page.waitForTimeout(80);
const afterList = await box('#notebookDetailScroll');
if (!before || !afterList || afterList.width <= before.width+150 ||
    await visibleControls() !== 2 ||
    overlap(await box('#sidebarRailToggle'), await box('#notebookPaneRestore'))) {
  throw new Error('list collapse');
}

await page.locator('#sidebarRailToggle').click();
await page.waitForFunction(() => state.sidebarCollapsed === true);
await page.waitForTimeout(80);
const afterBoth = await box('#notebookDetailScroll');
if (!afterBoth || afterBoth.width <= afterList.width+100 ||
    await visibleControls() !== 2 ||
    overlap(await box('#sidebarRailToggle'), await box('#notebookPaneRestore'))) {
  throw new Error('both collapse');
}

await page.locator('#notebookPaneRestore').click();
await page.locator('#sidebarRailToggle').click();
await page.waitForFunction(() => !state.notebookListCollapsed && !state.sidebarCollapsed);

const fav = await page.evaluate(async iid0 => {
  const x = await (await fetch('/api.php?action=content_get&id=' + iid0,{cache:'no-store'})).json();
  return Number(x.item?.is_favorite || 0);
}, iid);
...
if (/rgb\(247, 247, 247\)|rgb\(255, 255, 255\)/.test(dark.body + ' ' + dark.side)) {
  throw new Error('dark theme ' + JSON.stringify(dark));
}
await page.locator('#themeBtn').click();
...
for (const [w,h] of [[1920,1080],[1440,900],[1024,820],[768,900],[390,844]]) {
  ...
  if (dims.scroll > dims.client + 3) {
    throw new Error('horizontal overflow ' + w + ' ' + JSON.stringify(dims));
  }
}
...
console.log('P02_V250_UAUI_FINAL_PASS=' + JSON.stringify({
  typography,resize,before,afterList,afterBoth,dark,editorSize,errors
}));
await browser.close();
