...
mkdir -p "$EVID"
COOKIE=/tmp/p01-v2352-final.cookies
PIDFILE=/tmp/p01-v2352-final.pid
...
# 1. Isolated Owner-like immutable V2.35.1 runtime.
rm -rf "$ROOT" "$COOKIE"; cp -a source/src "$ROOT"; start_server
curl -fsS -c "$COOKIE" -b "$COOKIE" "http://127.0.0.1:${PORT}/setup.php" -o /tmp/p01-v2352-final-setup.html
CSRF=$(python3 - <<'PY'
import re
s=open('/tmp/p01-v2352-final-setup.html',encoding='utf-8').read();m=re.search(r'name="setup_csrf"\s+value="([^"]+)"',s);assert m;print(m.group(1))
PY
)
curl -fsS -c "$COOKIE" -b "$COOKIE" -X POST "http://127.0.0.1:${PORT}/setup.php" --data-urlencode "setup_csrf=$CSRF" --data-urlencode 'site_title=P01 V2352 Final Remote' --data-urlencode "admin_password=$ADMIN_PASS" --data-urlencode "admin_password_confirm=$ADMIN_PASS" >/dev/null
test "$(tr -d '\r\n' < "$ROOT/VERSION.txt")" = 2.35.1
grep -F "define('VF_VERSION', '2.35.1')" "$ROOT/app/bootstrap.php" >/dev/null
...
cat >/tmp/p01-v2352-final-seed.php <<'PHP'
<?php
...
$pub=$r->createCategory(['name'=>'V2352公开导航','description'=>'remote-public','is_private'=>false,'sort_order'=>100]);
$priv=$r->createCategory(['name'=>'V2352私人导航','description'=>'remote-private','is_private'=>true,'sort_order'=>90]);
$ids=[];
for($i=1;$i<=24;$i++){$x=$r->saveLink(null,['category_id'=>$i%6===0?$priv:$pub,'title'=>'V2352保留资源 '.$i,'url'=>'https://v2352-resource-'.$i.'.example.com','description'=>'preserve','tags'=>$i%3===0?'V2352,一致性':'V2352','is_private'=>$i%6===0,'is_favorite'=>$i%8===0],'manual');$ids[]=(int)$x['id'];}
foreach([['channels','频道'],['watch','电影'],['topics','AI']] as [$domain,$kind]){for($i=0;$i<4;$i++){$offset=$domain==='channels'?2:($domain==='watch'?7:12);$id=$ids[$offset+$i];$p=['surface'=>$domain,'resource_kind'=>$kind,'note'=>'preserve-'.$domain.'-'.$i];if($domain==='channels')$p['background_friendly']=true;if($domain==='watch'){$p['media_year']=2023+$i;$p['media_status']='want';}if($domain==='topics'){$p['source_kind']='remote_url';$p['source_ref']='https://v2352-topic-'.$i.'.example.com';}$s->upsertProfile($id,$p);}}
...
if($before['links']!==24||$before['categories']!==2||$before['favorites']!==3||$before['private']!==4||$before['profiles']!==14||$before['schema']!=='2026082901'||(int)($sc['start']??-1)!==12||(int)($sc['channels']??-1)!==4||(int)($sc['watch']??-1)!==4||(int)($sc['topics']??-1)!==4||(int)($sc['total']??-1)!==24)throw new RuntimeException('seed '.json_encode($before));
file_put_contents('/tmp/p01-v2352-final-before.json',json_encode($before,JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE));echo "SEED_PASS\n";
PHP
ROOT="$ROOT" php /tmp/p01-v2352-final-seed.php | grep -Fx SEED_PASS
cp /tmp/p01-v2352-final-before.json "$EVID/before.json"

# 3. Real updater discovery, authenticated Release download, prepare and Atomic install.
cat >/tmp/p01-v2352-final-update.php <<'PHP'
<?php
declare(strict_types=1);
$root=getenv('ROOT');require $root.'/app/bootstrap.php';require_once $root.'/app/UpdateManager.php';
$m=new VfUpdateManager(vf_db(),['root'=>$root,'private_root'=>VF_PRIVATE_ROOT,'current_version'=>'2.35.1']);
$c=$m->check(true);
if(($c['ok']??false)!==true||($c['current_version']??'')!=='2.35.1'||($c['latest_version']??'')!=='2.35.2'||($c['available']??false)!==true||($c['can_update']??false)!==true)throw new RuntimeException('check '.json_encode($c));
...
$p=$m->prepare();
if(($p['ok']??false)!==true||($p['from_version']??'')!=='2.35.1'||($p['to_version']??'')!=='2.35.2'||($p['release_tag']??'')!=='v2.35.2'||($p['asset_name']??'')!=='VF_Start_V2.35.2_UPDATE.zip'||(int)($p['update_package_bytes']??0)!==1377507||($p['update_package_sha256']??'')!=='0b518b5393a0fdefdd98924429ef16e2e0f85af366887fa8c8b5cb4b9bc73505')throw new RuntimeException('prepare '.json_encode($p));
...
if(($i['ok']??false)!==true||($i['updated']??false)!==true||($i['from_version']??'')!=='2.35.1'||($i['to_version']??'')!=='2.35.2')throw new RuntimeException('install '.json_encode($i));
...
PHP
ROOT="$ROOT" EVID="$EVID" php /tmp/p01-v2352-final-update.php | grep -Fx REMOTE_UPDATE_PASS

# 4. Post-upgrade exact bytes, data preservation, integrity, idempotent latest-state semantics.
test "$(tr -d '\r\n' < "$ROOT/VERSION.txt")" = 2.35.2
grep -F "define('VF_VERSION', '2.35.2')" "$ROOT/app/bootstrap.php" >/dev/null
for f in VERSION.txt app/bootstrap.php assets/update-core.js; do cmp "target/src/$f" "$ROOT/$f"; done
echo P01_V2352_REMOTE_RUNTIME_BYTES_MATCH_TAG=PASS | tee "$EVID/runtime-bytes.txt"
...
cat >/tmp/p01-v2352-final-post.php <<'PHP'
<?php
declare(strict_types=1);
$root=getenv('ROOT');$before=json_decode((string)file_get_contents('/tmp/p01-v2352-final-before.json'),true,512,JSON_THROW_ON_ERROR);require $root.'/app/bootstrap.php';require_once $root.'/app/SurfaceRepository.php';
...
PHP
ROOT="$ROOT" EVID="$EVID" php /tmp/p01-v2352-final-post.php | grep -Fx POST_PASS

cat >/tmp/p01-v2352-final-latest.php <<'PHP'
<?php
declare(strict_types=1);
$root=getenv('ROOT');require $root.'/app/bootstrap.php';require_once $root.'/app/UpdateManager.php';
$m=new VfUpdateManager(vf_db(),['root'=>$root,'private_root'=>VF_PRIVATE_ROOT,'current_version'=>'2.35.2']);
$c=$m->check(true);
if(($c['ok']??false)!==true||($c['current_version']??'')!=='2.35.2'||($c['latest_version']??'')!=='2.35.2'||($c['available']??true)!==false||($c['can_update']??true)!==false)throw new RuntimeException('latest check '.json_encode($c));
...
PHP
ROOT="$ROOT" EVID="$EVID" php /tmp/p01-v2352-final-latest.php | grep -Fx LATEST_STATE_PASS

# 5. Public/private and five-domain HTTP boundary after real remote update.
start_server
...
grep -F 'V2352公开导航' "$EVID/start-anonymous.html" >/dev/null
! grep -F 'V2352私人导航' "$EVID/start-anonymous.html" >/dev/null
cat >"$EVID/runtime-verdict.txt" <<EOF
P01_V2351_TO_V2352_REMOTE_ONLINE_UPDATE=PASS
P01_V2352_REMOTE_DISCOVERY=PASS
P01_V2352_REMOTE_RELEASE_DOWNLOAD=PASS
P01_V2352_DATA_PRESERVATION=PASS
P01_V2352_SQLITE_INTEGRITY=PASS
P01_V2352_PUBLIC_PRIVATE_HTTP=PASS
P01_V2352_FIVE_DOMAIN_HTTP=PASS
P01_V2352_RUNTIME_FEATURE_BYTES=PASS
P01_V2352_SCHEMA_UNCHANGED_2026082901=PASS
P01_V2352_LATEST_TERMINAL_STATE=PASS
