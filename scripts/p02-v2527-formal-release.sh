...
s=s.replace("'candidate_browser':'PASS_RUN_32206056733_PLUS_32146866564'","'candidate_browser':'PASS_RUN_32660166205'")
s=s.replace("'candidate_backend_data_privacy_transaction':'PASS_RUN_32206056733_PLUS_INHERITED_V2.5.3'","'candidate_backend_data_privacy_transaction':'PASS_RUN_32660166205'")
...
fwrite(STDERR,'FORCED_RESULT='.json_encode($failed,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)."\n");
...
fwrite(STDERR,'COMMIT_RESULT='.json_encode($committed,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)."\n");
...
if gh release view v2.5.27 --repo llhzx2018/vf-library >/dev/null 2>&1; then
  echo FORMAL_RELEASE_BLOCK=V2.5.27_ALREADY_EXISTS
  exit 31
fi
if gh api repos/llhzx2018/vf-library/git/ref/tags/v2.5.27 >/dev/null 2>&1; then
  echo FORMAL_TAG_BLOCK=V2.5.27_ALREADY_EXISTS
  exit 32
fi
gh release create v2.5.27 build/formal-a/* \
  --repo llhzx2018/vf-library \
  --target "$PRODUCT_REF" \
  --title 'VF Library V2.5.27' \
  --notes-file build/formal-a/VF_Library_V2.5.27_RELEASE_NOTES.md

gh release view v2.5.27 --repo llhzx2018/vf-library --json databaseId,tagName,isDraft,isPrerelease,publishedAt,targetCommitish >"$RUNNER_TEMP/release2527.json"
jq -e '.tagName=="v2.5.27" and .isDraft==false and .isPrerelease==false' "$RUNNER_TEMP/release2527.json" >/dev/null
mkdir -p "$RUNNER_TEMP/readback2527"
gh release download v2.5.27 --repo llhzx2018/vf-library --pattern "$UPDATE_NAME" --dir "$RUNNER_TEMP/readback2527"
RBYTES=$(stat -c%s "$RUNNER_TEMP/readback2527/$UPDATE_NAME")
RSHA=$(sha256sum "$RUNNER_TEMP/readback2527/$UPDATE_NAME"|awk '{print $1}')
test "$RBYTES" = "$(stat -c%s build/formal-a/$UPDATE_NAME)"
test "$RSHA" = "$(sha256sum build/formal-a/$UPDATE_NAME|awk '{print $1}')"
TAGSHA=$(gh api repos/llhzx2018/vf-library/git/ref/tags/v2.5.27 --jq .object.sha)
test "$TAGSHA" = "$PRODUCT_REF"
echo RELEASE_ID="$(jq -r .databaseId "$RUNNER_TEMP/release2527.json")"
echo PUBLISHED_AT="$(jq -r .publishedAt "$RUNNER_TEMP/release2527.json")"
