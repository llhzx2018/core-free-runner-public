...
dump_gate_logs(){
  rc=$?
  echo "P01_V24718_RELEASE_WRAPPER_RC=$rc" >&2
  for log in /tmp/p01-r18-*-evidence/server.log /tmp/p01-r18-public-ui-server.log; do
    if [ -f "$log" ]; then
      echo "===== $log =====" >&2
      cat "$log" >&2 || true
    fi
...
DST=/tmp/p01-v24718-release-gate.sh
cp "$SRC" "$DST"
...
pairs=[
("TARGET_SHA=421e7b5df1e48db768b39470fc99580330a07c0f","TARGET_SHA=52249a697b907e15f6cb56acb37fa8993b789d80"),
("TARGET_TREE=598eaae633ef485dd2eef1330a9370cb46f90506","TARGET_TREE=2b2570247d96aab6f4a33841f7c5dfd311697b33"),
("SOURCE_SHA=e714cfe540ad39202c2cb558e5c0a370ef6d6502","SOURCE_SHA=dfbafc51a55a37e52240f2b029f05ab311735f12"),
("SOURCE_VERSION=2.47.13","SOURCE_VERSION=2.47.17"),
("TARGET_VERSION=2.47.14","TARGET_VERSION=2.47.18"),
("OUT=/tmp/p01-v24714-navigation","OUT=/tmp/p01-v24718-navigation-round5"),
("BUILD=/tmp/p01-v24714-navigation-builder","BUILD=/tmp/p01-v24718-navigation-round5-builder"),
("VF-Start-V2.47.14-FULL.zip","VF-Start-V2.47.18-FULL.zip"),
("VF_Start_V2.47.14_UPDATE.zip","VF_Start_V2.47.18_UPDATE.zip"),
("repair-v2.47.14.php","repair-v2.47.18.php"),
("/tmp/p01-v24714-navigation-builder/gate.py","/tmp/p01-v24718-navigation-round5-builder/gate.py"),
('VERSION = "2.47.14"','VERSION = "2.47.18"'),
('SOURCE_VERSION = "2.47.13"','SOURCE_VERSION = "2.47.17"'),
('VF_VERSION!=="2.47.14"','VF_VERSION!=="2.47.18"'),
("public const SOURCE_VERSION='2.47.13';","public const SOURCE_VERSION='2.47.17';"),
("public const TARGET_VERSION='2.47.14';","public const TARGET_VERSION='2.47.18';"),
("V2.47.14 fixes logged-out Navigation row layout width while preserving Atomic update safety.","V2.47.18 restores mobile sort reachability while preserving Atomic update safety."),
("/tmp/p01-v24714-navigation-rebuild","/tmp/p01-v24718-navigation-round5-rebuild"),
("P01_V24714_NAVIGATION_ROW_RELEASE_GATE=PASS","P01_V24718_NAVIGATION_UX_ROUND5_RELEASE_GATE=PASS"),
("R14_GATE_RECEIPT.txt","R18_GATE_RECEIPT.txt"),
("V24713_TO_V24714_ATOMIC_UPGRADE=PASS","V24717_TO_V24718_ATOMIC_UPGRADE=PASS"),
("OWNER_PREVIEW_RUNTIME_REASON=bounded corrective layout bug; no new product shape or interaction contract","OWNER_PREVIEW_RUNTIME_REASON=bounded responsive control reachability fix; no new product shape, data, auth, or IA contract"),
]
for a,b in pairs:
    if a not in s:
        raise SystemExit("missing patch anchor: "+a)
    s=s.replace(a,b)
s=s.replace("p01-r14-","p01-r18-").replace("R14","R18").replace("r14","r18")
p.write_text(s,encoding='utf-8')
PY

bash "$DST"

# Stable Navigation regressions + Round 5 responsive sort acceptance.
...
OUT=/tmp/p01-v24718-navigation-round5
cat >>"$OUT/R18_GATE_RECEIPT.txt" <<'EOF'
