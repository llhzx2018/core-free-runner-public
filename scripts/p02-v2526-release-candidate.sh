...
cat >"$RUNNER_ROOT/v2526-browser.mjs" <<'JS'
...
seed_interaction() {
  local site="$1" out="$2"
  cat >"$RUNNER_TEMP/v2526-interaction-seed.php" <<'PHP'
<?php
...
PHP
  php "$RUNNER_TEMP/v2526-interaction-seed.php" "$site" "$out"
}

seed_interaction "$FRESH/site" "$FRESH/interaction.json"
ITEM_ID=$(jq -r .item_id "$FRESH/interaction.json")
BASE_URL="http://127.0.0.1:18324/" ITEM_ID="$ITEM_ID" TEST_PASSWORD="P02-V2526-FRESH-$GITHUB_RUN_ID!" node "$RUNNER_ROOT/v2526-browser.mjs"
...
BASE_URL="http://127.0.0.1:18325/" ITEM_ID="$UP_ITEM_ID" TEST_PASSWORD="P02-V2526-UP-$GITHUB_RUN_ID!" node "$RUNNER_ROOT/v2526-browser.mjs"
...
echo V2526_INTERACTION_REFRESH_AFTER_ATOMIC_UPGRADE=PASS

echo CANDIDATE_SOURCE_COMMIT="$(git rev-parse HEAD)"
echo CANDIDATE_SOURCE_TREE="$TREE"
echo REL_STATE=REL.READY_PREPARE_ONLY_V2523_TO_V2526
