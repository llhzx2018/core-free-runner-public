...
TARGET_SHA=252ca51215e7e4411741e2f53d0bcc68f37614f2
TARGET_TREE=1e6fe31f03ed871e68b3153b7e5c1e81099d1cf7
SOURCE_SHA=972541fd06c66044105ec1b19cf7738c340c90c1
SOURCE_VERSION=2.47.10
TARGET_VERSION=2.47.11
OUT=/tmp/p01-v24711-atomic-r1
BUILD=/tmp/p01-v24711-atomic-r1-builder
FULL=VF-Start-V2.47.11-FULL.zip
UPDATE=VF_Start_V2.47.11_UPDATE.zip
REPAIR=repair-v2.47.11.php
...
p=Path('/tmp/p01-v24711-atomic-r1-builder/gate.py')
...
build_once "$OUT"
build_once /tmp/p01-v24711-atomic-r1-rebuild
(cd "$OUT" && sha256sum "$FULL" "$UPDATE" "$REPAIR" | sort) >/tmp/p01-r1-a.sum
(cd /tmp/p01-v24711-atomic-r1-rebuild && sha256sum "$FULL" "$UPDATE" "$REPAIR" | sort) >/tmp/p01-r1-b.sum
...
FRESH="$FRESH" php -r 'require getenv("FRESH")."/app/bootstrap.php"; if(VF_VERSION!=="2.47.11") exit(1);'
...
P01_V24711_ATOMIC_HOTFIX_R1_GATE=PASS
EOF
