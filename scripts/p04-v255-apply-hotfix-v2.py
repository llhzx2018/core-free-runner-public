...
# Runtime identity only. V2.5.4 product runtime is otherwise preserved.
(root/'VERSION').write_text('2.5.5\n',encoding='utf-8')

# Release-tree authority recognizes 2.5.5 as the same maintenance family.
p=root/'scripts/build-release-tree.py'
s=p.read_text(encoding='utf-8')
old="maintenance_versions={'2.5.1','2.5.2','2.5.3','2.5.4'}"
new="maintenance_versions={'2.5.1','2.5.2','2.5.3','2.5.4','2.5.5'}"
if s.count(old)!=1: raise SystemExit('BUILD_RELEASE_TREE_MAINTENANCE_SENTINEL_DRIFT')
s=s.replace(old,new,1)
old="if version=='2.5.4':\n    version_note='V2.5.4 unifies the full backend UA/UI, fixes update-history final-outcome semantics, normalizes feedback/accessibility/responsive behavior, and keeps Schema/business/provider authority unchanged.'"
new="if version=='2.5.5':\n    version_note='V2.5.5 preserves the verified V2.5.4 backend UA/UI runtime and fixes the formal Online Handoff packaging contract; Schema/business/provider authority remains unchanged.'\nelif version=='2.5.4':\n    version_note='V2.5.4 unifies the full backend UA/UI, fixes update-history final-outcome semantics, normalizes feedback/accessibility/responsive behavior, and keeps Schema/business/provider authority unchanged.'"
if s.count(old)!=1: raise SystemExit('BUILD_RELEASE_TREE_NOTE_SENTINEL_DRIFT')
s=s.replace(old,new,1)
p.write_text(s,encoding='utf-8')

# Base Atomic builder is the template authority. Derive the Marker from target Runtime's
# UpdateContract instead of maintaining a second hard-coded contract.
...
if s.count(old)!=1: raise SystemExit('BASE_SELFTEST_SENTINEL_DRIFT')
s=s.replace(old,new,1)
...
# IMPORTANT: the base builder itself contains a Python f-string. It therefore needs
# two literal backslashes in builder source to emit one PHP namespace backslash.
ns=r'\\VFInfra\\Core\\Update\\OnlineUpdateHandoff'
old="$authReady=Auth::isAuthenticated();"
new=("$authReady=Auth::isAuthenticated();"
     "$onlineHandoffRequest=(string)($_SERVER['REQUEST_METHOD']??'GET')==='POST'&&array_key_exists('vf_online_handoff',$_POST);"
     "if($onlineHandoffRequest){{"
     "if(!$authReady)throw new RuntimeException('在线更新交接需要现有管理员会话。');"
     "Auth::verifySameOrigin();"
     "$handoffToken=trim((string)($_POST['vf_online_handoff']??''));"
     "if($handoffToken==='')throw new RuntimeException('在线更新交接凭据缺失。');"
     f"$onlinePlan={ns}::authorize(VF_INFRA_ATOMIC_TARGET,$handoffToken,basename(__FILE__));"
     "$onlineOperationId=(string)($onlinePlan['operation_id']??'');"
     "if($onlineOperationId==='')throw new RuntimeException('在线更新 operation_id 缺失。');"
     "}}else{{"
     f"$onlinePlan={ns}::readPlan();"
     "if(is_array($onlinePlan)&&hash_equals((string)($onlinePlan['target_version']??''),VF_INFRA_ATOMIC_TARGET)&&hash_equals((string)($onlinePlan['repair_file']??''),basename(__FILE__))&&!empty($onlinePlan['authorized_at']))$onlineOperationId=(string)($onlinePlan['operation_id']??'');"
     "}}")
...
new="$tx->commit();$journal->clear();if($onlineOperationId!=='')try{{"+ns+"::recordRepairResult($onlineOperationId,'success','',['schema'=>14]);}}catch(Throwable $ignored){{}}"
...
new="if($onlineOperationId!=='')try{{$onlineResult=($tx!==null&&$dbRestoreFailed==='')?'rolled_back':'failed';"+ns+"::recordRepairResult($onlineOperationId,$onlineResult,'atomic',['reason'=>vfi251_safe_error($e)]);}}catch(Throwable $ignored){{}}$state='failed';$headline=$dbRestoreFailed===''?'升级失败，已自动回滚':'升级失败，需要人工恢复';"
if s.count(old)!=1: raise SystemExit('BASE_FAILURE_SENTINEL_DRIFT')
s=s.replace(old,new,1)
p.write_text(s,encoding='utf-8')

# V2.5.5 update wrapper. It carries the complete V2.5.4 runtime delta because current
# Production is 2.5.3; no user must install the quarantined 2.5.4 first.
...
start=base.index('PAYLOAD_PATHS=['); end=base.index(']\n\n',start)+2
...
base=base.replace(needle,"manifest['update_and_atomic_bytes_identical']=atomic.read_bytes()==update.read_bytes()\n"+insert+"manifest_path.write_text",1)
(root/'scripts/build-v255-update-release.py').write_text(base,encoding='utf-8')

# Machine package gate. The expected marker is always read from target Runtime authority.
...
marker=m.group(1);raw=zpath.read_bytes();actual=hashlib.sha256(raw).hexdigest()
...
for required in [b'OnlineUpdateHandoff::authorize',b'OnlineUpdateHandoff::recordRepairResult',b'vf_online_handoff']:
    if required not in repair: raise SystemExit('HANDOFF_BEHAVIOR_MISSING')
...
with tempfile.TemporaryDirectory(prefix='p04-v255-validate-') as td:
    ...
    if subprocess.run(['php','-l',str(f)],capture_output=True).returncode!=0: raise SystemExit('MALFORMED_REPAIR_PHP')
    if subprocess.run(['php',str(f),'--self-test'],capture_output=True).returncode!=0: raise SystemExit('REPAIR_SELF_TEST_FAILED')
...
print('P04_V255_HOTFIX_PATCH_V2_APPLIED')
