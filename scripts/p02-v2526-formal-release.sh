...
SRCVER=2.5.23
SCHEMA=2401
UPDATE_NAME="VF_Library_V2.5.26_UPDATE.zip"
...
cp scripts/build-release-v2.5.4.py "$RUNNER_TEMP/build-v2526.py"
python3 - "$RUNNER_TEMP/build-v2526.py" <<'PY'
from pathlib import Path
import sys
p=Path(sys.argv[1]);s=p.read_text(encoding='utf-8')
old="ROOT=Path(__file__).resolve().parents[1]; SRCVER='2.5.2'; VER='2.5.4'; SCHEMA=2401; DT=(2026,8,19,2,0,0)"
new="ROOT=Path.cwd(); SRCVER='2.5.23'; VER='2.5.26'; SCHEMA=2401; DT=(2026,8,24,2,0,0)"
assert old in s
s=s.replace(old,new,1)
s=s.replace("default='build/release-v2.5.4'","default='build/release-v2.5.26'").replace("default='release/v2.5.4'","default='release/v2.5.26'")
...
V2.5.26 is the direct immutable Atomic upgrade for authenticated Production V2.5.23. It includes the favorite/category-ordering correction and the interaction refresh fixes.
...
s=s.replace("'candidate_browser':'PASS_RUN_32206056733_PLUS_32146866564'","'candidate_browser':'PASS_RUN_32658615220'")
s=s.replace("'candidate_backend_data_privacy_transaction':'PASS_RUN_32206056733_PLUS_INHERITED_V2.5.3'","'candidate_backend_data_privacy_transaction':'PASS_RUN_32658615220'")
...
PY
python3 -m py_compile "$RUNNER_TEMP/build-v2526.py"
TREE=$(git show -s --format=%T "$PRODUCT_REF")
python3 "$RUNNER_TEMP/build-v2526.py" --out build/formal-a --source-commit "$PRODUCT_REF" --source-tree "$TREE" --source-ref release/v2.5.26 >"$RUNNER_TEMP/build-a.json"
python3 "$RUNNER_TEMP/build-v2526.py" --out build/formal-b --source-commit "$PRODUCT_REF" --source-tree "$TREE" --source-ref release/v2.5.26 >"$RUNNER_TEMP/build-b.json"
...
assert ha==hb,(ha,hb)
req={'VF_Library_V2.5.26_UPDATE.zip','VF_Library_V2.5.26_FULL.zip','VF_Library_V2.5.26_ATOMIC.zip','VF_Library_V2.5.26_SOURCE.zip','repair-v2.5.26.php','VF_Library_V2.5.26_RELEASE_NOTES.md','VF_Library_V2.5.26_RELEASE_MANIFEST.json','SHA256SUMS.txt'}
assert req<=set(ha),set(ha)
m=json.load(open('build/formal-a/VF_Library_V2.5.26_RELEASE_MANIFEST.json',encoding='utf-8'))
assert m['version']=='2.5.26' and m['compatibility']['supported_from']==['2.5.23']
print('DETERMINISTIC_RELEASE_SET=PASS')
for n,h in sorted(ha.items()): print(n,h)
PY
for z in "$UPDATE_NAME" "VF_Library_V2.5.26_FULL.zip" "VF_Library_V2.5.26_ATOMIC.zip"; do unzip -t "build/formal-a/$z" >/dev/null; done
unzip -p "build/formal-a/$UPDATE_NAME" atomic-manifest.json | jq -e '.source_version=="2.5.23" and .target_version=="2.5.26" and .source_schema==2401 and .target_schema==2401' >/dev/null
...
FRESH="$RUNNER_TEMP/fresh2526"; mkdir -p "$FRESH/site"
unzip -q build/formal-a/VF_Library_V2.5.26_FULL.zip -d "$FRESH/site"
test "$(cat "$FRESH/site/VERSION.txt")" = 2.5.26
setup_site "$FRESH/site" "$FRESH" 18324 "P02-V2526-FRESH-$GITHUB_RUN_ID!"
...
php "$FRESH/site/cli/verify.php" | jq -e '.ok==true and .version=="2.5.26" and .schema_version==2401 and .integrity=="ok" and .foreign_key_errors==0' >/dev/null
kill "$SITE_PID"; wait "$SITE_PID" 2>/dev/null || true
echo FULL_FRESH_INSTALL_FAVORITE_ORDERING=PASS

UP="$RUNNER_TEMP/up2523"; mkdir -p "$UP"
git worktree add --detach "$UP/source" v2.5.23 >/dev/null
mkdir -p "$UP/site"; bash "$UP/source/scripts/build-deploy-tree.sh" "$UP/site" >/dev/null
test "$(cat "$UP/site/VERSION.txt")" = 2.5.23
setup_site "$UP/site" "$UP" 18325 "P02-V2526-UP-$GITHUB_RUN_ID!"
...
$cid=$org->save(null,['name'=>'V2526 Upgrade Preserve']);
...
$m=['schema_version'=>'1.0','project_id'=>'P02','component_id'=>'APP','enabled'=>true,'current_version'=>'2.5.23','target_version'=>'2.5.26','update_type'=>'ATOMIC','from_versions'=>['2.5.23'],'schema_from'=>'2401','schema_to'=>'2401','repository'=>'llhzx2018/vf-library','release_tag'=>'v2.5.26','asset_name'=>'VF_Library_V2.5.26_UPDATE.zip','asset_bytes'=>$bytes,'asset_sha256'=>$sha,'backup_required'=>true,'rollback_supported'=>true,'released_at'=>'2026-08-23T18:00:00Z'];
$c=new CoreUpdates\UpdateCore('P02','APP');if(($c->check('2.5.23','2401',$m)['status']??'')!=='AVAILABLE')exit(2);if(($c->verifyPackage($pkg,$m)['status']??'')!=='VERIFIED')exit(3);$res=$c->upgrade('2.5.23','2401',new VfLibraryCoreUpdateAdapter(),$pkg,$m);file_put_contents($out,json_encode($res));if(!in_array($res['status']??'',['COMMITTED','COMMITTED_WITH_CLEANUP_WARNING'],true)||empty($res['backup_locator']))exit(4);
PHP
php "$UP/upgrade.php" "$UP/site" "$PKG" "$BYTES" "$SHA" "$UP/result.json"
test "$(cat "$UP/site/VERSION.txt")" = 2.5.26
...
php "$UP/site/cli/verify.php" | jq -e '.ok==true and .version=="2.5.26" and .schema_version==2401 and .integrity=="ok" and .foreign_key_errors==0' >/dev/null
kill "$SITE_PID"; wait "$SITE_PID" 2>/dev/null || true
echo EXISTING_DATA_2.5.23_TO_2.5.26=PASS
echo AUTOMATIC_BACKUP_AND_DATA_PRESERVATION=PASS
echo FAVORITE_ORDERING_AFTER_ATOMIC_UPGRADE=PASS

echo FORMAL_SOURCE_COMMIT="$(git rev-parse HEAD)"
echo FORMAL_SOURCE_TREE="$TREE"
echo REL_STATE=REL.FORMAL_BUILD_VERIFIED
echo MAIN_PROMOTION=PASS

export GH_TOKEN="$RELEASE_TOKEN"
if gh release view v2.5.26 --repo llhzx2018/vf-library >/dev/null 2>&1; then
  echo FORMAL_RELEASE_BLOCK=V2.5.26_ALREADY_EXISTS
  exit 31
fi
if gh api repos/llhzx2018/vf-library/git/ref/tags/v2.5.26 >/dev/null 2>&1; then
  echo FORMAL_TAG_BLOCK=V2.5.26_ALREADY_EXISTS
  exit 32
fi
gh release create v2.5.26 build/formal-a/* \
  --repo llhzx2018/vf-library \
  --target "$PRODUCT_REF" \
  --title 'VF Library V2.5.26' \
  --notes-file build/formal-a/VF_Library_V2.5.26_RELEASE_NOTES.md

gh release view v2.5.26 --repo llhzx2018/vf-library --json databaseId,tagName,isDraft,isPrerelease,publishedAt,targetCommitish >"$RUNNER_TEMP/release2526.json"
jq -e '.tagName=="v2.5.26" and .isDraft==false and .isPrerelease==false' "$RUNNER_TEMP/release2526.json" >/dev/null
mkdir -p "$RUNNER_TEMP/readback2526"
gh release download v2.5.26 --repo llhzx2018/vf-library --pattern "$UPDATE_NAME" --dir "$RUNNER_TEMP/readback2526"
RBYTES=$(stat -c%s "$RUNNER_TEMP/readback2526/$UPDATE_NAME")
RSHA=$(sha256sum "$RUNNER_TEMP/readback2526/$UPDATE_NAME"|awk '{print $1}')
test "$RBYTES" = "$(stat -c%s build/formal-a/$UPDATE_NAME)"
test "$RSHA" = "$(sha256sum build/formal-a/$UPDATE_NAME|awk '{print $1}')"
TAGSHA=$(gh api repos/llhzx2018/vf-library/git/ref/tags/v2.5.26 --jq .object.sha)
test "$TAGSHA" = "$PRODUCT_REF"
echo RELEASE_ID="$(jq -r .databaseId "$RUNNER_TEMP/release2526.json")"
echo PUBLISHED_AT="$(jq -r .publishedAt "$RUNNER_TEMP/release2526.json")"
