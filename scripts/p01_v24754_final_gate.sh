...
test -f "$FULL"
test -f "$UPDATE"
rm -rf /tmp/p01-v24754-update
unzip -q "$UPDATE" -d /tmp/p01-v24754-update
REPAIR=/tmp/p01-v24754-update/repair-v2.47.54.php
test -f "$REPAIR"
php -l "$REPAIR" >/dev/null
php "$REPAIR" --self-test | tee /tmp/p01-v24754-atomic-selftest.json
grep -Fq '"ok":true' /tmp/p01-v24754-atomic-selftest.json
...
install_full "$FULL" /tmp/p01-v24754-full-runtime 18477 "VF Start V2.47.54 FULL Gate"
test "$(tr -d '\r\n ' </tmp/p01-v24754-full-runtime/VERSION.txt)" = "$TARGET"
echo P01_V24753_CLEAN_FULL_INSTALL=PASS

install_full "$REL/VF-Start-V2.47.53-FULL.zip" /tmp/p01-v24753-upgrade 18478 "VF Start V2.47.53 Upgrade Source"

cat >/tmp/p01-v24753-upgrade/sentinel.php <<'PHP'
<?php
...
PHP
php /tmp/p01-v24753-upgrade/sentinel.php
rm /tmp/p01-v24753-upgrade/sentinel.php

cp "$REPAIR" /tmp/p01-v24753-upgrade/repair-v2.47.54.php
php /tmp/p01-v24753-upgrade/repair-v2.47.54.php --verify-source=/tmp/p01-v24753-upgrade | tee /tmp/p01-source-verify.json
grep -Fq '"ok":true' /tmp/p01-source-verify.json

php /tmp/p01-v24753-upgrade/repair-v2.47.54.php --run=/tmp/p01-v24753-upgrade | tee /tmp/p01-atomic-success.json
grep -Fq '"ok":true' /tmp/p01-atomic-success.json
test "$(tr -d '\r\n ' </tmp/p01-v24753-upgrade/VERSION.txt)" = "$TARGET"
(cd /tmp/p01-v24753-upgrade && php cli/verify.php) | grep -Fx 'VERIFY_PASS=YES'
php -r 'require "/tmp/p01-v24753-upgrade/app/bootstrap.php"; $id=(int)trim(file_get_contents("/tmp/p01-sentinel-id.txt")); $s=vf_db()->prepare("SELECT title,is_private FROM links WHERE id=?"); $s->execute([$id]); $r=$s->fetch(PDO::FETCH_ASSOC); if(!$r||$r["title"]!=="Release Sentinel Link"||(int)$r["is_private"]!==1) exit(1); echo "SENTINEL_PRESERVED\n";'

cp "$REPAIR" /tmp/p01-v24753-upgrade/repair-v2.47.54.php
php /tmp/p01-v24753-upgrade/repair-v2.47.54.php --run=/tmp/p01-v24753-upgrade | tee /tmp/p01-atomic-idempotent.json
...
install_full "$REL/VF-Start-V2.47.53-FULL.zip" /tmp/p01-v24753-rollback 18479 "VF Start V2.47.53 Rollback Source"
cat >/tmp/p01-v24753-rollback/sentinel.php <<'PHP'
<?php
declare(strict_types=1);
require __DIR__.'/app/bootstrap.php';
$db=vf_db();
...
PHP
php /tmp/p01-v24753-rollback/sentinel.php
rm /tmp/p01-v24753-rollback/sentinel.php
cp "$REPAIR" /tmp/p01-v24753-rollback/repair-v2.47.54.php
set +e
VF_ATOMIC_TEST_FAIL_AFTER_APPLY=1 php /tmp/p01-v24753-rollback/repair-v2.47.54.php --run=/tmp/p01-v24753-rollback >/tmp/p01-atomic-failure.out 2>/tmp/p01-atomic-failure.err
status=$?
set -e
test "$status" -ne 0
test "$(tr -d '\r\n ' </tmp/p01-v24753-rollback/VERSION.txt)" = "$SOURCE"
(cd /tmp/p01-v24753-rollback && php cli/verify.php) | grep -Fx 'VERIFY_PASS=YES'
php -r 'require "/tmp/p01-v24753-rollback/app/bootstrap.php"; $id=(int)trim(file_get_contents("/tmp/p01-rollback-id.txt")); $s=vf_db()->prepare("SELECT title FROM links WHERE id=?"); $s->execute([$id]); if($s->fetchColumn()!=="Rollback Sentinel Link") exit(1); echo "ROLLBACK_SENTINEL_PRESERVED\n";'
echo P01_V24753_ATOMIC_ROLLBACK=PASS

unzip -p "$FULL" VERSION.txt | tr -d '\r\n ' | grep -Fx "$TARGET"
unzip -p "$FULL" release-manifest.json | php -r '$m=json_decode(stream_get_contents(STDIN),true); if(($m["version"]??"")!=="2.47.54"||($m["source_version"]??"")!=="2.47.53"||($m["schema_version"]??"")!=="2026090401")exit(1); echo "CANDIDATE_MANIFEST_IDENTITY=PASS\n";'
(
  cd "$ART"
  sha256sum -c "VF-Start-V2.47.54-FULL.zip.sha256"
  sha256sum -c "VF_Start_V2.47.54_UPDATE.zip.sha256"
)
