...
mkdir -p "$EVID"
COOKIE=/tmp/p01-v2351-final.cookies
PIDFILE=/tmp/p01-v2351-final.pid
...
# 1. Build isolated Owner-like V2.35.0 runtime and install normally.
rm -rf "$ROOT" "$COOKIE"; cp -a source/src "$ROOT"; start_server
curl -fsS -c "$COOKIE" -b "$COOKIE" "http://127.0.0.1:${PORT}/setup.php" -o /tmp/p01-v2351-final-setup.html
CSRF=$(python3 - <<'PY'
import re
s=open('/tmp/p01-v2351-final-setup.html',encoding='utf-8').read();m=re.search(r'name="setup_csrf"\s+value="([^"]+)"',s);assert m;print(m.group(1))
PY
)
curl -fsS -c "$COOKIE" -b "$COOKIE" -X POST "http://127.0.0.1:${PORT}/setup.php" --data-urlencode "setup_csrf=$CSRF" --data-urlencode 'site_title=P01 V2351 Final Remote' --data-urlencode "admin_password=$ADMIN_PASS" --data-urlencode "admin_password_confirm=$ADMIN_PASS" >/dev/null
...
cat >/tmp/p01-v2351-final-seed.php <<'PHP'
<?php
...
$pub=$r->createCategory(['name'=>'V2351公开导航','description'=>'remote-public','is_private'=>false,'sort_order'=>100]);
$priv=$r->createCategory(['name'=>'V2351私人导航','description'=>'remote-private','is_private'=>true,'sort_order'=>90]);
$ids=[];
for($i=1;$i<=20;$i++){$x=$r->saveLink(null,['category_id'=>$i%5===0?$priv:$pub,'title'=>'V2351保留资源 '.$i,'url'=>'https://v2351-resource-'.$i.'.example.com','description'=>'preserve','tags'=>$i%3===0?'V2351,一致性':'V2351','is_private'=>$i%5===0,'is_favorite'=>$i%7===0],'manual');$ids[]=(int)$x['id'];}
foreach([['channels','频道'],['watch','电影'],['topics','AI']] as [$domain,$kind]){for($i=0;$i<3;$i++){$id=$ids[3+$i+(($domain==='channels'?0:($domain==='watch'?3:6)))];$p=['surface'=>$domain,'resource_kind'=>$kind,'note'=>'preserve-'.$domain.'-'.$i];if($domain==='channels')$p['background_friendly']=true;if($domain==='watch'){$p['media_year']=2024+$i;$p['media_status']='want';}if($domain==='topics'){$p['source_kind']='remote_url';$p['source_ref']='https://v2351-topic-'.$i.'.example.com';}$s->upsertProfile($id,$p);}}
...
file_put_contents('/tmp/p01-v2351-final-before.json',json_encode($before,JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE));echo "SEED_PASS\n";
PHP
ROOT="$ROOT" php /tmp/p01-v2351-final-seed.php | grep -Fx SEED_PASS
cp /tmp/p01-v2351-final-before.json "$EVID/before.json"

# 3. Real updater discovery/prepare/install through published core-updates/main + v2.35.1 Release.
cat >/tmp/p01-v2351-final-update.php <<'PHP'
<?php
declare(strict_types=1);
$root=getenv('ROOT');require $root.'/app/bootstrap.php';require_once $root.'/app/UpdateManager.php';
$m=new VfUpdateManager(vf_db(),['root'=>$root,'private_root'=>VF_PRIVATE_ROOT,'current_version'=>'2.35.0']);
$c=$m->check(true);
if(($c['ok']??false)!==true||($c['current_version']??'')!=='2.35.0'||($c['latest_version']??'')!=='2.35.1'||($c['available']??false)!==true||($c['can_update']??false)!==true)throw new RuntimeException('check '.json_encode($c));
...
$p=$m->prepare();
if(($p['ok']??false)!==true||($p['from_version']??'')!=='2.35.0'||($p['to_version']??'')!=='2.35.1'||($p['release_tag']??'')!=='v2.35.1'||($p['asset_name']??'')!=='VF_Start_V2.35.1_UPDATE.zip'||(int)($p['update_package_bytes']??0)!==1376805||($p['update_package_sha256']??'')!=='e6e40b77d75319f24ea4e0e803863a1b5ffffcdf2f3405d62dc9399cbc7114af')throw new RuntimeException('prepare '.json_encode($p));
...
if(($i['ok']??false)!==true||($i['updated']??false)!==true||($i['from_version']??'')!=='2.35.0'||($i['to_version']??'')!=='2.35.1')throw new RuntimeException('install '.json_encode($i));
...
PHP
ROOT="$ROOT" EVID="$EVID" php /tmp/p01-v2351-final-update.php | grep -Fx REMOTE_UPDATE_PASS

# 4. Post-upgrade exact version/schema/data/integrity and V2.35.1 runtime delta binding.
test "$(tr -d '\r\n' < "$ROOT/VERSION.txt")" = 2.35.1
grep -F "define('VF_VERSION', '2.35.1')" "$ROOT/app/bootstrap.php" >/dev/null
for f in VERSION.txt app/FunctionalHome.php app/bootstrap.php assets/resource-media.css assets/workspace-home.css; do
  cmp "target/src/$f" "$ROOT/$f"
done
echo P01_V2351_REMOTE_RUNTIME_BYTES_MATCH_TAG=PASS | tee "$EVID/runtime-bytes.txt"
...
cat >/tmp/p01-v2351-final-post.php <<'PHP'
<?php
declare(strict_types=1);
$root=getenv('ROOT');$before=json_decode((string)file_get_contents('/tmp/p01-v2351-final-before.json'),true,512,JSON_THROW_ON_ERROR);require $root.'/app/bootstrap.php';require_once $root.'/app/SurfaceRepository.php';
...
PHP
ROOT="$ROOT" EVID="$EVID" php /tmp/p01-v2351-final-post.php | grep -Fx POST_PASS
...
grep -F 'V2351公开导航' "$EVID/start-anonymous.html" >/dev/null
! grep -F 'V2351私人导航' "$EVID/start-anonymous.html" >/dev/null
cat >"$EVID/runtime-verdict.txt" <<EOF
P01_V2350_TO_V2351_REMOTE_ONLINE_UPDATE=PASS
P01_V2351_REMOTE_DISCOVERY=PASS
P01_V2351_REMOTE_RELEASE_DOWNLOAD=PASS
P01_V2351_DATA_PRESERVATION=PASS
P01_V2351_SQLITE_INTEGRITY=PASS
P01_V2351_PUBLIC_PRIVATE_HTTP=PASS
P01_V2351_FIVE_DOMAIN_HTTP=PASS
P01_V2351_RUNTIME_FEATURE_BYTES=PASS
