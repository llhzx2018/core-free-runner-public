...
python3 scripts/p01_build_v24741_formal.py | tee /tmp/p01-v24741-build.json

FULL="$ART/VF-Start-V2.47.41-FULL.zip"
UPDATE="$ART/VF_Start_V2.47.41_UPDATE.zip"
REPAIR="$ART/repair-v2.47.41.php"

test -f "$FULL"
test -f "$UPDATE"
unzip -q "$UPDATE" -d /tmp/p01-v24741-update
REPAIR=/tmp/p01-v24741-update/repair-v2.47.41.php
test -f "$REPAIR"
php -l "$REPAIR" >/dev/null
php "$REPAIR" --self-test | tee /tmp/p01-v24741-atomic-selftest.json
grep -Fq '"ok":true' /tmp/p01-v24741-atomic-selftest.json
...
install_full "$FULL" /tmp/p01-v24741-full-runtime 18463 "VF Start V2.47.41 FULL Gate"
...
PHP
php /tmp/p01-v24740-upgrade/sentinel.php
rm /tmp/p01-v24740-upgrade/sentinel.php

cp "$REPAIR" /tmp/p01-v24740-upgrade/repair-v2.47.41.php
php /tmp/p01-v24740-upgrade/repair-v2.47.41.php --verify-source=/tmp/p01-v24740-upgrade | tee /tmp/p01-source-verify.json
grep -Fq '"ok":true' /tmp/p01-source-verify.json

php /tmp/p01-v24740-upgrade/repair-v2.47.41.php --run=/tmp/p01-v24740-upgrade | tee /tmp/p01-atomic-success.json
grep -Fq '"ok":true' /tmp/p01-atomic-success.json
test "$(tr -d '\r\n ' </tmp/p01-v24740-upgrade/VERSION.txt)" = "$TARGET"
(cd /tmp/p01-v24740-upgrade && php cli/verify.php) | grep -Fx 'VERIFY_PASS=YES'
php -r 'require "/tmp/p01-v24740-upgrade/app/bootstrap.php"; $id=(int)trim(file_get_contents("/tmp/p01-sentinel-id.txt")); $s=vf_db()->prepare("SELECT title,is_private FROM links WHERE id=?"); $s->execute([$id]); $r=$s->fetch(PDO::FETCH_ASSOC); if(!$r||$r["title"]!=="Release Sentinel Link"||(int)$r["is_private"]!==1) exit(1); echo "SENTINEL_PRESERVED\n";'

# Idempotency on already-current runtime.
cp "$REPAIR" /tmp/p01-v24740-upgrade/repair-v2.47.41.php
php /tmp/p01-v24740-upgrade/repair-v2.47.41.php --run=/tmp/p01-v24740-upgrade | tee /tmp/p01-atomic-idempotent.json
grep -Fq '"already_current":true' /tmp/p01-atomic-idempotent.json
echo P01_V24741_ATOMIC_UPGRADE=PASS
...
PHP
php /tmp/p01-v24740-rollback/sentinel.php
rm /tmp/p01-v24740-rollback/sentinel.php
cp "$REPAIR" /tmp/p01-v24740-rollback/repair-v2.47.41.php
set +e
VF_ATOMIC_TEST_FAIL_AFTER_APPLY=1 php /tmp/p01-v24740-rollback/repair-v2.47.41.php --run=/tmp/p01-v24740-rollback >/tmp/p01-atomic-failure.out 2>/tmp/p01-atomic-failure.err
status=$?
set -e
test "$status" -ne 0
test "$(tr -d '\r\n ' </tmp/p01-v24740-rollback/VERSION.txt)" = "$SOURCE"
(cd /tmp/p01-v24740-rollback && php cli/verify.php) | grep -Fx 'VERIFY_PASS=YES'
php -r 'require "/tmp/p01-v24740-rollback/app/bootstrap.php"; $id=(int)trim(file_get_contents("/tmp/p01-rollback-id.txt")); $s=vf_db()->prepare("SELECT title FROM links WHERE id=?"); $s->execute([$id]); if($s->fetchColumn()!=="Rollback Sentinel Link") exit(1); echo "ROLLBACK_SENTINEL_PRESERVED\n";'
echo P01_V24741_ATOMIC_ROLLBACK=PASS

# Artifact identity/readback.
unzip -p "$FULL" VERSION.txt | tr -d '\r\n ' | grep -Fx "$TARGET"
unzip -p "$FULL" release-manifest.json | php -r '$m=json_decode(stream_get_contents(STDIN),true); if(($m["version"]??"")!=="2.47.41"||($m["source_version"]??"")!=="2.47.40"||($m["schema_version"]??"")!=="2026090401")exit(1); echo "FORMAL_MANIFEST_IDENTITY=PASS\n";'
(
  cd "$ART"
  sha256sum -c "VF-Start-V2.47.41-FULL.zip.sha256"
  sha256sum -c "VF_Start_V2.47.41_UPDATE.zip.sha256"
)
echo P01_V24741_FORMAL_ARTIFACT_GATE=PASS
