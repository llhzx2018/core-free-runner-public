...
new='''build_fixture "$RT" "$SUFFIX"; start_fixture "$RT" "$DATA" "$PORT"; trap 'stop_fixture 18238' EXIT; setup_login "$PORT" "$DATA" "$COOKIE" "$RT"; sed -i "s/define('VFAB_SCHEMA_VERSION', 30);/define('VFAB_SCHEMA_VERSION', 29);/" "$RT/app/bootstrap.php"; grep -q "define('VFAB_SCHEMA_VERSION', 29);" "$RT/app/bootstrap.php"; code=$(curl -sS -b "$COOKIE" -o "$GATE_ROOT/wrong-schema.html" -w '%{http_code}' "http://127.0.0.1:$PORT/$TOOL"); echo "WRONG_SCHEMA_HTTP=$code"; sed -n '1,20p' "$GATE_ROOT/wrong-schema.html"; test "$code" = 409; grep -q 'Current Version/Schema must be exactly 1.35.4/30' "$GATE_ROOT/wrong-schema.html"; test ! -e "$RT/memory-api.php"; test -e "$RT/$TOOL"; echo 'WRONG_SCHEMA=DENY'; stop_fixture "$PORT"; trap - EXIT'''
count=s.count(old)
