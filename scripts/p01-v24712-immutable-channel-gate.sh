...
CORE_SHA="efdae0300bd2b0d3f1a1ed1e9b31e9f8abe9cada"
P01_SHA="7eae0c4649a0eb5e1af8f68c4c82a923f236aed1"
RELEASE_ID="391380197"
UPDATE_NAME="VF_Start_V2.47.12_UPDATE.zip"
...
(
  ...
  find update-core/php -name '*.php' -print0 | xargs -0 -n1 php -l >/tmp/p01-v24712-core-php-lint.log
...
assert p['target_version']=='2.47.12'
...
assert p['release_tag']=='v2.47.12'
assert p['release_id']==391380197
assert p['product_identity']=='7eae0c4649a0eb5e1af8f68c4c82a923f236aed1'
assert p['asset_name']=='VF_Start_V2.47.12_UPDATE.zip'
assert p['asset_bytes']==1883036
assert p['asset_sha256']=='c6b262d3c2236847c3dbfec9b61b2d469852ec6a4f5e8bade1b3b3c6f6dc503a'
assert p['backup_required'] is True and p['rollback_supported'] is True
assert p['minimum_php']=='8.0.0'
print('P01_V24712_CHANNEL_MANIFEST_CONTRACT=PASS')
PY
)

REL="$(GH_TOKEN="$VF_RELEASE_WRITE_TOKEN" gh api repos/llhzx2018/vf-start/releases/tags/v2.47.12)"
...
$private = sys_get_temp_dir() . '/p01-v24712-freshness-' . bin2hex(random_bytes(4));
...
$db->prepare('INSERT INTO update_state(state_key,state_value,updated_at) VALUES(?,?,?)')->execute([
    'highest_verified_release_version', '2.47.11', gmdate('c')
]);
$db->prepare('INSERT INTO update_state(state_key,state_value,updated_at) VALUES(?,?,?)')->execute([
...
echo "STRICTLY_NEWER_THAN_OBSERVED_24711=PASS\n";

$blocked = false;
try {
    $m->invoke($manager, ['target_version'=>'2.47.11'], str_repeat('b',64));
} catch (Throwable $e) {
...
cat >evidence/runner/P01_V24712_IMMUTABLE_CHANNEL_GATE.txt <<EOF
CORE_UPDATES_EXACT_SHA=$CORE_SHA
P01_EXACT_SOURCE_SHA=$P01_SHA
CURRENT_VERSION=2.47.10
PREVIOUS_OBSERVED_TARGET=2.47.11
...
STRICTLY_NEWER_THAN_OBSERVED_24711=PASS
SAME_VERSION_SILENT_REPLACEMENT_GUARD=PRESERVED
PRODUCTION=NOT_TESTED
P01_V24712_IMMUTABLE_CHANNEL_GATE=PASS
EOF
cat evidence/runner/P01_V24712_IMMUTABLE_CHANNEL_GATE.txt
