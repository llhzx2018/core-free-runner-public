...
SOURCE_COMMIT=8f1073120f8096fc23554906615601752feb036d
TARGET_COMMIT=225780b1ed56bbcc8e62ba465b1129e1c22ee60a
mkdir -p "$EVID"
COOKIE=/tmp/p01-v2360-final.cookies
PIDFILE=/tmp/p01-v2360-final.pid
...
# 1. Isolated Owner-like immutable V2.35.3 runtime.
rm -rf "$ROOT" "$COOKIE"; cp -a source/src "$ROOT"; start_server
curl -fsS -c "$COOKIE" -b "$COOKIE" "http://127.0.0.1:${PORT}/setup.php" -o /tmp/p01-v2360-final-setup.html
CSRF=$(python3 - <<'PY'
import re
s=open('/tmp/p01-v2360-final-setup.html',encoding='utf-8').read();m=re.search(r'name="setup_csrf"\s+value="([^"]+)"',s);assert m;print(m.group(1))
PY
)
curl -fsS -c "$COOKIE" -b "$COOKIE" -X POST "http://127.0.0.1:${PORT}/setup.php" --data-urlencode "setup_csrf=$CSRF" --data-urlencode 'site_title=P01 V2360 Final Remote' --data-urlencode "admin_password=$ADMIN_PASS" --data-urlencode "admin_password_confirm=$ADMIN_PASS" >/dev/null
...
cat >/tmp/p01-v2360-final-seed.php <<'PHP'
<?php
...
$pub=$r->createCategory(['name'=>'V2360公开导航','description'=>'remote-public','is_private'=>false,'sort_order'=>100]);
$priv=$r->createCategory(['name'=>'V2360私人导航','description'=>'remote-private','is_private'=>true,'sort_order'=>90]);
$ids=[];
for($i=1;$i<=24;$i++){$x=$r->saveLink(null,['category_id'=>$i%6===0?$priv:$pub,'title'=>'V2360保留资源 '.$i,'url'=>'https://v2360-resource-'.$i.'.example.com','description'=>'preserve','tags'=>$i%3===0?'V2360,一致性':'V2360','is_private'=>$i%6===0,'is_favorite'=>$i%8===0],'manual');$ids[]=(int)$x['id'];}
foreach([['channels','频道'],['watch','电影'],['topics','AI']] as [$domain,$kind]){for($i=0;$i<4;$i++){$offset=$domain==='channels'?2:($domain==='watch'?7:12);$id=$ids[$offset+$i];$p=['surface'=>$domain,'resource_kind'=>$kind,'note'=>'preserve-'.$domain.'-'.$i];if($domain==='channels')$p['background_friendly']=true;if($domain==='watch'){$p['media_year']=2023+$i;$p['media_status']='want';}if($domain==='topics'){$p['source_kind']='remote_url';$p['source_ref']='https://v2360-topic-'.$i.'.example.com';}$s->upsertProfile($id,$p);}}
...
file_put_contents('/tmp/p01-v2360-final-before.json',json_encode($before,JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE));echo "SEED_PASS\n";
PHP
ROOT="$ROOT" php /tmp/p01-v2360-final-seed.php | grep -Fx SEED_PASS
cp /tmp/p01-v2360-final-before.json "$EVID/before.json"

# 3. Real updater discovery, published Release download, prepare and Atomic install.
cat >/tmp/p01-v2360-final-update.php <<'PHP'
<?php
declare(strict_types=1);
...
$c=$m->check(true);
if(($c['ok']??false)!==true||($c['current_version']??'')!=='2.35.3'||($c['latest_version']??'')!=='2.36.0'||($c['available']??false)!==true||($c['can_update']??false)!==true)throw new RuntimeException('check '.json_encode($c));
...
$p=$m->prepare();
if(($p['ok']??false)!==true||($p['from_version']??'')!=='2.35.3'||($p['to_version']??'')!=='2.36.0'||($p['release_tag']??'')!=='v2.36.0'||($p['asset_name']??'')!=='VF_Start_V2.36.0_UPDATE.zip'||(int)($p['update_package_bytes']??0)!==1404941||($p['update_package_sha256']??'')!=='26daa0fefff8a867a554750fbd4a92106fb2f9b1f68014cf8dd214d837fe0ee6')throw new RuntimeException('prepare '.json_encode($p));
...
if(($i['ok']??false)!==true||($i['updated']??false)!==true||($i['from_version']??'')!=='2.35.3'||($i['to_version']??'')!=='2.36.0')throw new RuntimeException('install '.json_encode($i));
...
PHP
ROOT="$ROOT" EVID="$EVID" php /tmp/p01-v2360-final-update.php | grep -Fx REMOTE_UPDATE_PASS

# 4. Post-upgrade all changed runtime bytes, data preservation and integrity.
...
while IFS=$'\t' read -r status p1 p2; do
  ...
  case "$status" in
    D*) test ! -e "$ROOT/$rel" ;;
    *) test -f "target/$path"; test -f "$ROOT/$rel"; cmp "target/$path" "$ROOT/$rel" ;;
  esac
done < "$EVID/runtime-delta.txt"
for f in app/ResourceCoverCache.php app/ResourceMetadata.php resource-cover-refresh.php; do test -f "$ROOT/$f"; cmp "target/src/$f" "$ROOT/$f"; done
echo P01_V2360_REMOTE_RUNTIME_DELTA_MATCH_TAG=PASS | tee "$EVID/runtime-bytes.txt"
...
cat >/tmp/p01-v2360-final-post.php <<'PHP'
<?php
declare(strict_types=1);
$root=getenv('ROOT');$before=json_decode((string)file_get_contents('/tmp/p01-v2360-final-before.json'),true,512,JSON_THROW_ON_ERROR);require $root.'/app/bootstrap.php';require_once $root.'/app/SurfaceRepository.php';
...
PHP
ROOT="$ROOT" EVID="$EVID" php /tmp/p01-v2360-final-post.php | grep -Fx POST_PASS

# 5. New manager must converge to current==latest terminal state.
cat >/tmp/p01-v2360-final-latest.php <<'PHP'
<?php
declare(strict_types=1);
$root=getenv('ROOT');require $root.'/app/bootstrap.php';require_once $root.'/app/UpdateManager.php';
$m=new VfUpdateManager(vf_db(),['root'=>$root,'private_root'=>VF_PRIVATE_ROOT,'current_version'=>'2.36.0']);
$c=$m->check(true);
if(($c['ok']??false)!==true||($c['current_version']??'')!=='2.36.0'||($c['latest_version']??'')!=='2.36.0'||($c['available']??true)!==false||($c['can_update']??true)!==false)throw new RuntimeException('latest check '.json_encode($c));
...
PHP
ROOT="$ROOT" EVID="$EVID" php /tmp/p01-v2360-final-latest.php | grep -Fx LATEST_STATE_PASS

# 6. Public/private and five-domain HTTP boundary after real remote update.
start_server
...
grep -F 'V2360公开导航' "$EVID/start-anonymous.html" >/dev/null
! grep -F 'V2360私人导航' "$EVID/start-anonymous.html" >/dev/null
cat >"$EVID/runtime-verdict.txt" <<EOF
P01_V2353_TO_V2360_REMOTE_ONLINE_UPDATE=PASS
P01_V2360_REMOTE_DISCOVERY=PASS
P01_V2360_REMOTE_RELEASE_DOWNLOAD=PASS
P01_V2360_DATA_PRESERVATION=PASS
P01_V2360_SQLITE_INTEGRITY=PASS
P01_V2360_PUBLIC_PRIVATE_HTTP=PASS
P01_V2360_FIVE_DOMAIN_HTTP=PASS
P01_V2360_RUNTIME_FEATURE_BYTES=PASS
P01_V2360_SCHEMA_UNCHANGED_2026082901=PASS
P01_V2360_LATEST_TERMINAL_STATE=PASS
