...
const base = process.env.VF_E2E_BASE_URL || 'http://127.0.0.1:19065';
...
if (!evidence || !candidate || !webRoot) throw new Error('mobile search-result audit environment missing');
fs.mkdirSync(evidence, { recursive: true });

const password = 'Vf' + crypto.randomUUID().replaceAll('-', '') + 'Aa1';
const query = 'v260-edge-01';
const report = {
  schema: 'p04-mobile-search-result-handoff-audit/v1',
...
async function pointerClick(locator) {
  ...
  if (!box || box.width <= 0 || box.height <= 0) throw new Error('search-result pointer target has no box');
...
try {
  ...
  await page.locator('#site_name').fill('VF Infra Search Result Handoff Audit');
  ...
  assert(report.result.input_value === query, `search input lost query before result action: ${report.result.input_value}`);
  assert(report.result.result_text.includes(query), `matching server result missing: ${report.result.result_text}`);
  assert(report.result.action_height >= 40, `search result action too short ${report.result.action_height}`);
  assert(report.result.action_width >= 44, `search result action too narrow ${report.result.action_width}`);
  assert(report.result.overflow <= 1, `search result overflow ${report.result.overflow}`);

  await pointerClick(current.action);
  await page.waitForFunction(() => location.hash.startsWith('#server/'), null, { timeout: 12000 });
  const serverHeading = page.locator('#v270-app h1').first();
  await serverHeading.waitFor({ state: 'visible', timeout: 15000 });
  report.result.detail_hash = await page.evaluate(() => location.hash);
  report.result.detail_h1 = clean(await serverHeading.innerText());
  assert(report.result.detail_h1.includes(query), `server detail h1 mismatch ${report.result.detail_h1}`);
  ...
  assert(report.browser_back.input_value === query, `browser Back lost search input: ${report.browser_back.input_value}`);
  assert(report.browser_back.result_text.includes(query), 'browser Back lost matching result');
  assert(report.browser_back.overflow <= 1, `browser Back overflow ${report.browser_back.overflow}`);
  await page.screenshot({ path: `${evidence}/01-search-after-browser-back.png`, fullPage: true, animations: 'disabled' });
  ...
  assert(target === `#search/${query}`, `detail context return target mismatch ${target}`);
  ...
  assert(report.context_back.action_height >= 40, `detail context back too short ${report.context_back.action_height}`);
  assert(report.context_back.action_width >= 44, `detail context back too narrow ${report.context_back.action_width}`);
  ...
  assert(report.context_back.input_value === query, `context Back lost search input: ${report.context_back.input_value}`);
  assert(report.context_back.result_text.includes(query), 'context Back lost matching result');
  assert(report.context_back.overflow <= 1, `context Back overflow ${report.context_back.overflow}`);
  await page.screenshot({ path: `${evidence}/02-search-after-context-back.png`, fullPage: true, animations: 'disabled' });
  ...
  fs.writeFileSync(`${evidence}/P04_MOBILE_SEARCH_RESULT_HANDOFF_AUDIT.json`, JSON.stringify(report, null, 2) + '\n');
  await browser.close();
}

console.log(`P04_MOBILE_SEARCH_RESULT_HANDOFF_AUDIT=${report.status}`);
