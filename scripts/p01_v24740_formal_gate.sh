...
ART=/tmp/p01-v24740-artifacts
REL39=/tmp/p01-rel39
TARGET=2.47.40
SOURCE=2.47.39
SCHEMA=2026090401

python3 scripts/p01_build_v24740_formal.py | tee /tmp/p01-v24740-build.json

FULL="$ART/VF-Start-V2.47.40-FULL.zip"
UPDATE="$ART/VF_Start_V2.47.40_UPDATE.zip"
REPAIR="$ART/repair-v2.47.40.php"

test -f "$FULL"
test -f "$UPDATE"
unzip -q "$UPDATE" -d /tmp/p01-v24740-update
REPAIR=/tmp/p01-v24740-update/repair-v2.47.40.php
test -f "$REPAIR"
php -l "$REPAIR" >/dev/null
php "$REPAIR" --self-test | tee /tmp/p01-v24740-atomic-selftest.json
grep -Fq '"ok":true' /tmp/p01-v24740-atomic-selftest.json
...
install_full "$FULL" /tmp/p01-v24740-full-runtime 18453 "VF Start V2.47.40 FULL Gate"
test "$(tr -d '\r\n ' </tmp/p01-v24740-full-runtime/VERSION.txt)" = "$TARGET"
echo P01_V24740_CLEAN_FULL_INSTALL=PASS

# Build a real installed V2.47.39 source for Atomic verification.
install_full "$REL39/VF-Start-V2.47.39-FULL.zip" /tmp/p01-v24739-upgrade 18454 "VF Start V2.47.39 Upgrade Source"

cat >/tmp/p01-v24739-upgrade/sentinel.php <<'PHP'
<?php
...
PHP
php /tmp/p01-v24739-upgrade/sentinel.php
rm /tmp/p01-v24739-upgrade/sentinel.php

cp "$REPAIR" /tmp/p01-v24739-upgrade/repair-v2.47.40.php
php /tmp/p01-v24739-upgrade/repair-v2.47.40.php --verify-source=/tmp/p01-v24739-upgrade | tee /tmp/p01-source-verify.json
grep -Fq '"ok":true' /tmp/p01-source-verify.json

php /tmp/p01-v24739-upgrade/repair-v2.47.40.php --run=/tmp/p01-v24739-upgrade | tee /tmp/p01-atomic-success.json
grep -Fq '"ok":true' /tmp/p01-atomic-success.json
test "$(tr -d '\r\n ' </tmp/p01-v24739-upgrade/VERSION.txt)" = "$TARGET"
(cd /tmp/p01-v24739-upgrade && php cli/verify.php) | grep -Fx 'VERIFY_PASS=YES'
php -r 'require "/tmp/p01-v24739-upgrade/app/bootstrap.php"; $id=(int)trim(file_get_contents("/tmp/p01-sentinel-id.txt")); $s=vf_db()->prepare("SELECT title,is_private FROM links WHERE id=?"); $s->execute([$id]); $r=$s->fetch(PDO::FETCH_ASSOC); if(!$r||$r["title"]!=="Release Sentinel Link"||(int)$r["is_private"]!==1) exit(1); echo "SENTINEL_PRESERVED\n";'

# Idempotency on already-current runtime.
cp "$REPAIR" /tmp/p01-v24739-upgrade/repair-v2.47.40.php
php /tmp/p01-v24739-upgrade/repair-v2.47.40.php --run=/tmp/p01-v24739-upgrade | tee /tmp/p01-atomic-idempotent.json
grep -Fq '"already_current":true' /tmp/p01-atomic-idempotent.json
echo P01_V24740_ATOMIC_UPGRADE=PASS
echo P01_V24740_ATOMIC_IDEMPOTENCY=PASS

# Failure/rollback gate from a separate exact V2.47.39 install.
install_full "$REL39/VF-Start-V2.47.39-FULL.zip" /tmp/p01-v24739-rollback 18455 "VF Start V2.47.39 Rollback Source"
cat >/tmp/p01-v24739-rollback/sentinel.php <<'PHP'
<?php
declare(strict_types=1);
require __DIR__.'/app/bootstrap.php';
$db=vf_db();
...
PHP
php /tmp/p01-v24739-rollback/sentinel.php
rm /tmp/p01-v24739-rollback/sentinel.php
cp "$REPAIR" /tmp/p01-v24739-rollback/repair-v2.47.40.php
set +e
VF_ATOMIC_TEST_FAIL_AFTER_APPLY=1 php /tmp/p01-v24739-rollback/repair-v2.47.40.php --run=/tmp/p01-v24739-rollback >/tmp/p01-atomic-failure.out 2>/tmp/p01-atomic-failure.err
status=$?
set -e
test "$status" -ne 0
test "$(tr -d '\r\n ' </tmp/p01-v24739-rollback/VERSION.txt)" = "$SOURCE"
(cd /tmp/p01-v24739-rollback && php cli/verify.php) | grep -Fx 'VERIFY_PASS=YES'
php -r 'require "/tmp/p01-v24739-rollback/app/bootstrap.php"; $id=(int)trim(file_get_contents("/tmp/p01-rollback-id.txt")); $s=vf_db()->prepare("SELECT title FROM links WHERE id=?"); $s->execute([$id]); if($s->fetchColumn()!=="Rollback Sentinel Link") exit(1); echo "ROLLBACK_SENTINEL_PRESERVED\n";'
echo P01_V24740_ATOMIC_ROLLBACK=PASS
