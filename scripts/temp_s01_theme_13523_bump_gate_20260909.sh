...
if git diff --name-only "$MAIN_SHA" "$BASE_SHA" | grep -E '(^|/)(update|migration|schema|recovery|secrets?|credentials?)(/|\.|$)'; then echo FAIL_RELEASE_SURFACE_BOUNDARY; exit 1; fi
...
for p in "${identity[@]}"; do case "$p" in VERSION|src/style.css|tests/*.php) ;; *) echo "FAIL_UNEXPECTED_VERSION_IDENTITY=$p"; exit 1;; esac; done
...
for raw in sys.argv[1:]:
    p=Path(raw); s=p.read_text()
    s=s.replace('V1.35.22_S01_ADMIN_PAGE_BY_PAGE_UX_CLOSURE','V1.35.23_S01_MATURE_ADMIN_PAGE1_5_PREVIEW').replace('1.35.22','1.35.23')
    p.write_text(s)
PY
test "$(tr -d '\r\n' < VERSION)" = '1.35.23'
...
for js in src/assets/js/admin/admin-dashboard.js src/assets/js/admin/admin-brand.js src/assets/js/admin/admin-brand-page-refinement-v1.js src/assets/js/admin/admin-layout.js src/assets/js/admin/admin-layout-page-refinement-v1.js src/assets/js/admin/admin-navigation.js src/assets/js/admin/admin-navigation-page-refinement-v1.js src/assets/js/admin/admin-render.js src/assets/js/admin/admin-preview.js src/assets/js/admin/admin-recovery-v510.js; do node --check "$js" >/dev/null; done
for t in tests/mature-product-v5-contract.php tests/workbench-page-refresh-parity-contract.php tests/brand-page-live-preview-contract.php tests/layout-page-workflow-contract.php tests/navigation-page-refinement-contract.php tests/render-page-refinement-contract.php tests/update-center-human-language-contract.php tests/workbench-primary-action-contract.php tests/final-admin-human-closure-contract.php tests/deep-admin-human-language-contract.php tests/layout-workflow-v4-contract.php tests/navigation-workflow-v4-contract.php tests/render-workflow-v4-contract.php tests/render-settings-human-language-contract.php tests/seo-workflow-v4-contract.php tests/preview-workflow-v4-contract.php tests/preview-human-language-followup-contract.php tests/recovery-workflow-v4-contract.php tests/recovery-human-language-closure-contract.php tests/update-refresh-policy-contract.php tests/visible-console-v2-contract.php; do php "$t"; done
