...
printf '%s\n' P01_V232_HOME_SOURCE_FENCE=PASS P01_V232_FIVE_FILE_DELTA=PASS P01_V232_VERSION_UNCHANGED_2.31.0=PASS | tee "$EVID/source.txt"

# 2. Fresh installed runtime using the real setup session.
rm -f "$COOKIE"
php -S "127.0.0.1:${PORT}" -t "$ROOT" >"$EVID/server.log" 2>&1 & echo $! >"$PIDFILE"
for i in $(seq 1 50); do if curl -fsS -c "$COOKIE" -b "$COOKIE" "http://127.0.0.1:${PORT}/setup.php" -o /tmp/p01-v232-v4-setup.html; then break; fi; sleep .25; done
test -s /tmp/p01-v232-v4-setup.html
CSRF=$(python3 - <<'PY'
import re
s=open('/tmp/p01-v232-v4-setup.html',encoding='utf-8').read();m=re.search(r'name="setup_csrf"\s+value="([^"]+)"',s);assert m;print(m.group(1))
PY
)
curl -fsS -c "$COOKIE" -b "$COOKIE" -X POST "http://127.0.0.1:${PORT}/setup.php" --data-urlencode "setup_csrf=$CSRF" --data-urlencode 'site_title=P01 V232 Home Gate V4' --data-urlencode "admin_password=$ADMIN_PASS" --data-urlencode "admin_password_confirm=$ADMIN_PASS" -o "$EVID/setup-post.html"
...
# 3. Runner-only, authenticated, self-deleting category fixture.
cat >"$ROOT/__p01_v232_home_gate_seed.php" <<'PHP'
<?php
...
try { $id=(new VfRepository(vf_db()))->createCategory(['name'=>'V232 Home Gate 分类','description'=>'runner-only fixture','is_private'=>false,'sort_order'=>100]); echo "CATEGORY_ID=".(int)$id."\n"; }
...
mkdir -p /tmp/p01-v232-browser-v4 && cd /tmp/p01-v232-browser-v4
...
const seed=await d.request.get(base+'/__p01_v232_home_gate_seed.php');if(seed.status()!==200)throw new Error('fixture '+seed.status());const seedText=(await seed.text()).trim();if(!/^CATEGORY_ID=\d+$/.test(seedText))throw new Error('fixture body '+seedText);fs.writeFileSync(e+'/fixture.txt',seedText+'\n');
const p=await d.newPage();await p.goto(base+'/home.php',{waitUntil:'networkidle'});if(!p.url().endsWith('/home.php'))throw new Error('home route '+p.url());
...
if(await p.locator('.vf-home-command').count()!==1)throw new Error('home command missing');if(await p.locator('.vf-sidebar-scope-section:visible').count()!==0)throw new Error('home scope visible');if(await p.locator('a[href="surfaces.php"]:visible').count()<1)throw new Error('all resources entry missing');
...
await p.getByRole('link',{name:'查看全部资源'}).click();await p.waitForLoadState('networkidle');if(!p.url().includes('/surfaces.php')||!(await p.locator('h1').first().innerText()).includes('全部资源'))throw new Error('all resources separation');if(await p.locator('.vf-global-domain-nav a.active').count()!==0)throw new Error('all impersonates home/domain');
...
P01_V232_RUNNER_ONLY_FIXTURE_SELF_DELETED=PASS
