...
import base64, hashlib, json, re, shutil, stat, zipfile
from pathlib import Path

ROOT=Path.cwd()
CAND=ROOT/"candidate"/"src"
BASE=Path("/tmp/p01-rel40/full")
OLD_REPAIR=Path("/tmp/p01-rel40/update/repair-v2.47.40.php")
OUT=Path("/tmp/p01-v24741-artifacts")
VERSION="2.47.41"
SOURCE_VERSION="2.47.40"
SCHEMA="2026090401"
SOURCE_SHA="e860c52a6be09fa2087e93c9d3f88795f0edeb13"
SOURCE_TREE="f6bc6d1d0216b7a76a564985f415f351c7a2b3c3"
...
def zip_deterministic(path:Path, files:dict[str,bytes]):
    with zipfile.ZipFile(path,"w",compression=zipfile.ZIP_DEFLATED,compresslevel=9) as z:
        for rel in sorted(files):
            info=zipfile.ZipInfo(rel,(2026,9,22,0,0,0))
...
old_source=decode_const(old,"SOURCE_MANIFEST")
old_target=decode_const(old,"TARGET_MANIFEST")
old_payload=decode_const(old,"PAYLOAD")
old_removed=decode_const(old,"REMOVED")
try:
    old_alts=decode_const(old,"SOURCE_ALTERNATES")
except SystemExit:
    old_alts={}
...
# Formal artifact metadata is generated from the previous formal manifest, not stale repository package metadata.
...
formal["release_scope"]="p01-v24741-formal-preflight"
...
repair=old
for a,b in [
    ("public const SOURCE_VERSION='2.47.39';",f"public const SOURCE_VERSION='{SOURCE_VERSION}';"),
    ("public const TARGET_VERSION='2.47.40';",f"public const TARGET_VERSION='{VERSION}';"),
...
(OUT/"P01_V24741_ARTIFACT_RECEIPT.json").write_text(json.dumps(receipt,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
