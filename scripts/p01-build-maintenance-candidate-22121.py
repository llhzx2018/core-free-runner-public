...
EXPECTED_ADDED = {
    'assets/data-recovery.js',
    'assets/update-reload.js',
}
EXPECTED_RUNTIME_DELTA = {
    ...
    'assets/reference-ui.js',
    'assets/sidebar-refinement.css',
    'assets/update-reload.js',
    'assets/update.js',
    'data-safety.php',
    'governance.php',
...
base = load_module('p01_base_22121', HERE / 'p01-build-release.py')
v2 = load_module('p01_v2_atomic_22121', HERE / 'p01-build-release-v2.py')
...
def main() -> None:
    ...
    if added != EXPECTED_ADDED or removed:
        raise SystemExit(f'unexpected runtime shape drift added={sorted(added)} removed={sorted(removed)}')

    changed = {
        k for k in target_runtime
        if k not in source_runtime or sha256_bytes(target_runtime[k]) != sha256_bytes(source_runtime[k])
    }
    ...
    notes = (
        ...
        '- Scope: navigation stability + backend value simplification + update reload\n'
