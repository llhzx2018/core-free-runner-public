...
ROOT=Path.cwd()
VERSION='2.27.0'
SOURCE_VERSION='2.26.0'
SCHEMA='2026082801'
CANDIDATE='8c3d18e1243d4b6ddc40bc7922746131a5d0d9c3'
TREE='6e9fb22d146b6b1e9ab276100331043c8ec647aa'
SOURCE='c97a362fc574cd80349526fca016f9e0e6a9c472'
...
for r in ['app/SurfaceShell.php','assets/surface-workspace.css','assets/workspace.js','workspace-action.php','surfaces.php','start.php','channels.php','watch.php','surface-manager.php','app/SurfaceRepository.php','cli/surface-verify.php','cli/baseline-verify.php']:
...
manifest={
 ...
 'browser_extension':{'version':'1.6.4','release_unit':'INDEPENDENT','released_this_round':False},
 'update':{'project_id':'P01','component_id':'APP','manifest_truth':'llhzx2018/core-updates/projects/P01.json','release_truth':'GitHub Release','asset_name':'VF_Start_V2.27.0_UPDATE.zip','supported_from':[SOURCE_VERSION],'backup_required':True,'rollback_supported':True},
 ...
 'workspace_productization':{'add_modal':True,'detail_drawer':True,'bulk_actions':True,'pagination':True,'sorting':True,'list_card_modes':True,'anonymous_public_isolated':True},
 'presentation_authority':{'interaction':'SINGLE_WORKSPACE','visual':'VF_ADMIN_SHELL_TEAL','workspace_asset':'assets/surface-workspace.css'},
...
new="public const SOURCE_VERSION='2.26.0';\n    public const TARGET_VERSION='2.27.0';\n    public const TARGET_SCHEMA='2026082801';"
if repair.count(old)!=1: raise SystemExit('repair constant anchor mismatch')
repair=repair.replace(old,new,1)
out=Path('/tmp/p01-v227-artifacts'); out.mkdir(parents=True,exist_ok=True)
rp=out/'repair-v2.27.0.php'; rp.write_text(repair,encoding='utf-8',newline='\n')
base.deterministic_zip(out/'VF-Start-V2.27.0-FULL.zip',full)
base.deterministic_zip(out/'VF-Start-V2.27.0-UPDATE.zip',{rp.name:rp.read_bytes()})
base.deterministic_zip(out/'VF_Start_V2.27.0_UPDATE.zip',{rp.name:rp.read_bytes()})
...
(out/'P01-V2.27.0-ARTIFACT-GATE.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
