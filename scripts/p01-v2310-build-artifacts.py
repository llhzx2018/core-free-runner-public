...
if changed != expected_delta:
    raise SystemExit('unexpected V2.31 runtime delta: ' + json.dumps(changed))
if added:
    raise SystemExit('unexpected V2.31 added runtime files: ' + json.dumps(added))
if removed:
    raise SystemExit('unexpected V2.31 removed runtime files: ' + json.dumps(removed))
...
manifest = {
    ...
    'update': {
        ...
        'asset_name': 'VF_Start_V2.31.0_UPDATE.zip',
        'supported_from': [SOURCE_VERSION], 'backup_required': True,
        'rollback_supported': True, 'schema_migration_atomic': False,
    },
    'ux_v231': {
        'context_aware_create_form': True,
        'server_side_domain_normalization': True,
        'private_state_semantic': True,
        'contextual_action_copy': True,
        'public_private_contract_unchanged': True,
    },
...
new = "public const SOURCE_VERSION='2.30.0';\n    public const TARGET_VERSION='2.31.0';\n    public const TARGET_SCHEMA='2026082901';"
if repair.count(old) != 1:
    raise SystemExit('repair constant anchor mismatch')
repair = repair.replace(old, new, 1)

out = Path('/tmp/p01-v2310-artifacts')
out.mkdir(parents=True, exist_ok=True)
repair_path = out / 'repair-v2.31.0.php'
repair_path.write_text(repair, encoding='utf-8', newline='\n')
base.deterministic_zip(out / 'VF-Start-V2.31.0-FULL.zip', full)
base.deterministic_zip(out / 'VF_Start_V2.31.0_UPDATE.zip', {repair_path.name: repair_path.read_bytes()})
result = {
...
(out / 'P01-V2.31.0-FORMAL-GATE.json').write_text(json.dumps(result, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
