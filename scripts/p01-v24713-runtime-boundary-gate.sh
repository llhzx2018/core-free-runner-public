...
TARGET_SHA=e714cfe540ad39202c2cb558e5c0a370ef6d6502
TARGET_TREE=400768a3b11edba4e0b54264a08b08ec6af9155e
SOURCE_SHA=972541fd06c66044105ec1b19cf7738c340c90c1
SOURCE_VERSION=2.47.10
TARGET_VERSION=2.47.13
OUT=/tmp/p01-v24713-boundary
BUILD=/tmp/p01-v24713-boundary-builder
FULL=VF-Start-V2.47.13-FULL.zip
UPDATE=VF_Start_V2.47.13_UPDATE.zip
REPAIR=repair-v2.47.13.php
...
p=Path('/tmp/p01-v24713-boundary-builder/gate.py')
s=p.read_text(encoding='utf-8')
if 'VERSION = "2.47.11"' not in s: raise SystemExit('old harness version anchor mismatch')
s=s.replace('VERSION = "2.47.11"', 'VERSION = "2.47.13"', 1)
s=s.replace('V2.47.11 restores private update authorization recovery and stale-discovery handling.', 'V2.47.13 restores the proven V2.47.10 Atomic runtime boundary while preserving strict runtime hash verification.', 1)
...
assert old in s
s=s.replace(old,new,1)
# Restore the proven V2.47.10 Atomic runtime boundary: deployment/docs stay in FULL but are not runtime source gates.
...
if old not in s: raise SystemExit('runtime boundary collection anchor mismatch')
s=s.replace(old,new,1)
old="""        'private_data_included': False,"""
new="""        'private_data_included': False,
        'atomic_runtime_excluded_files': sorted(runtime_excluded),"""
if old not in s: raise SystemExit('runtime boundary manifest anchor mismatch')
s=s.replace(old,new,1)
...
if old not in s: raise SystemExit('runtime boundary full target anchor mismatch')
s=s.replace(old,new,1)
old="""    base.deterministic_zip(out / FULL_NAME, target_with)"""
new="""    base.deterministic_zip(out / FULL_NAME, full_with)"""
if old not in s: raise SystemExit('runtime boundary full zip anchor mismatch')
s=s.replace(old,new,1)
...
build_once "$OUT"
build_once /tmp/p01-v24713-boundary-rebuild
(cd "$OUT" && sha256sum "$FULL" "$UPDATE" "$REPAIR" | sort) >/tmp/p01-r1-a.sum
(cd /tmp/p01-v24713-boundary-rebuild && sha256sum "$FULL" "$UPDATE" "$REPAIR" | sort) >/tmp/p01-r1-b.sum
...
FRESH="$FRESH" php -r 'require getenv("FRESH")."/app/bootstrap.php"; if(VF_VERSION!=="2.47.13") exit(1);'
...
for f in .gitignore CHANGELOG.md DEPLOY-HERE.txt FULL-PACKAGE-NOTES.txt README.md UPGRADE-V2.txt robots.txt; do printf 'OWNER-PRODUCTION-GATE-ONLY-DRIFT:%s\n' "$f" >"$UP/$f"; done
...
TAMPER=/tmp/p01-r1-runtime-tamper; rm -rf "$TAMPER"; cp -a "$BASELINE_ROOT/src" "$TAMPER"
bridge_state "$TAMPER"
printf '\n/* runtime tamper probe */\n' >>"$TAMPER/app/bootstrap.php"
set +e
php "$OUT/$REPAIR" --verify-source="$TAMPER" >/tmp/p01-r1-runtime-tamper.json
tamper_rc=$?
set -e
test "$tamper_rc" -ne 0
jq -e '.ok==false and (.errors|index("app/bootstrap.php:sha")!=null)' /tmp/p01-r1-runtime-tamper.json >/dev/null
...
P01_V24713_RUNTIME_BOUNDARY_GATE=PASS
EOF
