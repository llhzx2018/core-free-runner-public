...
SCHEMA='2026082801'
CANDIDATE='c97a362fc574cd80349526fca016f9e0e6a9c472'
TREE='784fbbb4b3612145cdfa424659ddc6e843d3694b'
SOURCE='894cc9cb647a9b4694b0684d4bba89497e182e66'
...
for r in ['app/SurfaceShell.php','assets/surface-workspace.css','surfaces.php','start.php','channels.php','watch.php','surface-manager.php','system-baseline.php','cli/baseline-verify.php','app/SurfaceRepository.php','cli/surface-verify.php']:
...
manifest={
 ...
 'update':{'project_id':'P01','component_id':'APP','manifest_truth':'llhzx2018/core-updates/projects/P01.json','release_truth':'GitHub Release','asset_name':'VF_Start_V2.26.0_UPDATE.zip','supported_from':[SOURCE_VERSION],'backup_required':True,'rollback_supported':True},
 ...
 'presentation_authority':{'interaction':'RAINDROP_SINGLE_WORKSPACE','visual':'VF_ADMIN_SHELL_TEAL','workspace_asset':'assets/surface-workspace.css'},
...
full['release-manifest.json']=mb
atomic=dict(target)
atomic['release-manifest.json']=mb
...
new="public const SOURCE_VERSION='2.25.0';\n    public const TARGET_VERSION='2.26.0';\n    public const TARGET_SCHEMA='2026082801';"
if repair.count(old)!=1: raise SystemExit('repair constant anchor mismatch')
repair=repair.replace(old,new,1)
out=Path('/tmp/p01-v226-artifacts')
out.mkdir(parents=True,exist_ok=True)
rp=out/'repair-v2.26.0.php'
rp.write_text(repair,encoding='utf-8',newline='\n')
base.deterministic_zip(out/'VF-Start-V2.26.0-FULL.zip',full)
base.deterministic_zip(out/'VF-Start-V2.26.0-UPDATE.zip',{rp.name:rp.read_bytes()})
base.deterministic_zip(out/'VF_Start_V2.26.0_UPDATE.zip',{rp.name:rp.read_bytes()})
result={
 'project_id':'P01','version':VERSION,'source_version':SOURCE_VERSION,
 'candidate':CANDIDATE,'candidate_tree':TREE,'source_commit':SOURCE,
 ...
 'atomic_update':True,'status':'BUILD_PASS'
}
(out/'P01-V2.26.0-ARTIFACT-GATE.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
