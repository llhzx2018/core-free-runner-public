...
dump_gate_logs(){
  rc=$?
  echo "P01_V24717_RELEASE_WRAPPER_RC=$rc" >&2
  for log in /tmp/p01-r17-*-evidence/server.log /tmp/p01-r17-public-ui-server.log; do
    if [ -f "$log" ]; then
      echo "===== $log =====" >&2
      cat "$log" >&2 || true
    fi
...
DST=/tmp/p01-v24717-release-gate.sh
cp "$SRC" "$DST"
...
pairs=[
("TARGET_SHA=421e7b5df1e48db768b39470fc99580330a07c0f","TARGET_SHA=dfbafc51a55a37e52240f2b029f05ab311735f12"),
("TARGET_TREE=598eaae633ef485dd2eef1330a9370cb46f90506","TARGET_TREE=712118f6b2b26f6e868c305324ffb1414cf7a6d7"),
("SOURCE_SHA=e714cfe540ad39202c2cb558e5c0a370ef6d6502","SOURCE_SHA=f953988b4001f04b36c027168a815d80ae3b5fb7"),
("SOURCE_VERSION=2.47.13","SOURCE_VERSION=2.47.16"),
("TARGET_VERSION=2.47.14","TARGET_VERSION=2.47.17"),
("OUT=/tmp/p01-v24714-navigation","OUT=/tmp/p01-v24717-navigation-round4"),
("BUILD=/tmp/p01-v24714-navigation-builder","BUILD=/tmp/p01-v24717-navigation-round4-builder"),
("VF-Start-V2.47.14-FULL.zip","VF-Start-V2.47.17-FULL.zip"),
("VF_Start_V2.47.14_UPDATE.zip","VF_Start_V2.47.17_UPDATE.zip"),
("repair-v2.47.14.php","repair-v2.47.17.php"),
("/tmp/p01-v24714-navigation-builder/gate.py","/tmp/p01-v24717-navigation-round4-builder/gate.py"),
('VERSION = "2.47.14"','VERSION = "2.47.17"'),
('SOURCE_VERSION = "2.47.13"','SOURCE_VERSION = "2.47.16"'),
('VF_VERSION!=="2.47.14"','VF_VERSION!=="2.47.17"'),
("public const SOURCE_VERSION='2.47.13';\\\\n    public const TARGET_VERSION='2.47.14';","public const SOURCE_VERSION='2.47.16';\\\\n    public const TARGET_VERSION='2.47.17';"),
("V2.47.14 fixes logged-out Navigation row layout width while preserving Atomic update safety.","V2.47.17 adds server-rendered global account shell ownership simplification while preserving Atomic update safety."),
...
for a,b in pairs:
    ...
    s=s.replace(a,b)
s=s.replace("p01-r14-","p01-r17-").replace("R14","R17").replace("r14","r17")
p.write_text(s,encoding='utf-8')
PY

bash "$DST"

# Round 2 regression + Round 4 exact-source focused acceptance.
...
OUT=/tmp/p01-v24717-navigation-round4
cat >>"$OUT/R17_GATE_RECEIPT.txt" <<'EOF'
...
for test_name in \
  ...
  global_account_actions_contract.php \
  subject_domain_nav_shell_contract.php; do
...
FRESH_OWNER_ANONYMOUS_SHELL=PASS
