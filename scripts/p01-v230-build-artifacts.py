...
expected_delta = sorted([
    ...
    'app/bootstrap.php',
    'assets/workspace-domain-nav.css',
    'assets/workspace-rebaseline.css',
    'assets/workspace-rebaseline.js',
])
if changed != expected_delta:
    raise SystemExit('unexpected V2.30 runtime delta: ' + json.dumps(changed))
if added != ['assets/workspace-domain-nav.css']:
    raise SystemExit('unexpected V2.30 added runtime files: ' + json.dumps(added))
if removed:
    raise SystemExit('unexpected V2.30 removed runtime files: ' + json.dumps(removed))
...
manifest = {
    ...
    'update': {
        ...
        'asset_name': 'VF_Start_V2.30.0_UPDATE.zip',
    ...
    'resource_domains': {
        ...
        'labels': ['导航', '频道', '影视', '专题'],
        'navigation_category_table': 'categories',
        'non_navigation_taxonomy': 'resource_kind+tags',
    },
    'ux_v230': {
        'global_domain_navigation': True,
        'navigation_taxonomy': True,
        'domain_specific_content_density': True,
        'mobile_command_row': True,
        'large_library_450_gate': True,
...
new = "public const SOURCE_VERSION='2.29.0';\n    public const TARGET_VERSION='2.30.0';\n    public const TARGET_SCHEMA='2026082901';"
if repair.count(old) != 1:
    raise SystemExit('repair constant anchor mismatch')
repair = repair.replace(old, new, 1)

out = Path('/tmp/p01-v230-artifacts')
out.mkdir(parents=True, exist_ok=True)
repair_path = out / 'repair-v2.30.0.php'
repair_path.write_text(repair, encoding='utf-8', newline='\n')
base.deterministic_zip(out / 'VF-Start-V2.30.0-FULL.zip', full)
base.deterministic_zip(out / 'VF_Start_V2.30.0_UPDATE.zip', {repair_path.name: repair_path.read_bytes()})
result = {
...
(out / 'P01-V2.30.0-FORMAL-GATE.json').write_text(json.dumps(result, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
