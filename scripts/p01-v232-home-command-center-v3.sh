...
mkdir -p "$EVID"
COOKIE=/tmp/p01-v232-home-v3.cookies
PIDFILE=/tmp/p01-v232-home-v3.pid
cleanup(){ rm -f "$ROOT/__p01_v232_home_gate_seed.php"; if test -f "$PIDFILE"; then kill "$(cat "$PIDFILE")" >/dev/null 2>&1 || true; fi; }
trap cleanup EXIT

# 1. Exact product source and scope. Product bytes are unchanged from the first Home candidate.
...
# 2. Fresh web runtime. Keep the setup session cookie so setup is a real committed install.
rm -f "$COOKIE"
php -S "127.0.0.1:${PORT}" -t "$ROOT" >"$EVID/server.log" 2>&1 & echo $! >"$PIDFILE"
for i in $(seq 1 50); do
  if curl -fsS -c "$COOKIE" -b "$COOKIE" "http://127.0.0.1:${PORT}/setup.php" -o /tmp/p01-v232-v3-setup.html; then break; fi
  sleep .25
done
test -s /tmp/p01-v232-v3-setup.html
CSRF=$(python3 - <<'PY'
import re
s=open('/tmp/p01-v232-v3-setup.html',encoding='utf-8').read();m=re.search(r'name="setup_csrf"\s+value="([^"]+)"',s);assert m;print(m.group(1))
PY
)
curl -fsS -c "$COOKIE" -b "$COOKIE" -X POST "http://127.0.0.1:${PORT}/setup.php" \
  --data-urlencode "setup_csrf=$CSRF" \
  --data-urlencode 'site_title=P01 V232 Home Gate V3' \
...
# 3. Runner-only one-shot fixture. It is written into the ephemeral checkout, requires an authenticated admin session, and deletes itself after use.
...
try {
    ...
    $id = $repo->createCategory(['name'=>'V232 Home Gate 分类','description'=>'runner-only fixture','is_private'=>false,'sort_order'=>100]);
    echo "CATEGORY_ID=" . (int)$id . "\n";
} finally {
    @unlink(__FILE__);
}
PHP
...
# 4. Real desktop/mobile browser verification and real authenticated mutations.
mkdir -p /tmp/p01-v232-browser-v3 && cd /tmp/p01-v232-browser-v3
...
const totalText=await p.locator('.vf-home-status-grid a').filter({hasText:'全部资源'}).innerText();const totalNum=Number((totalText.match(/\b(\d+)\b/)||[])[1]||0);if(totalNum<4)throw new Error('total '+totalText);
const favText=await p.locator('.vf-home-status-grid a').filter({hasText:'我的收藏'}).innerText();const favNum=Number((favText.match(/\b(\d+)\b/)||[])[1]||0);if(favNum<1)throw new Error('favorite '+favText);
