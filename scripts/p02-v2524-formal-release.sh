...
if gh release view v2.5.24 --repo llhzx2018/vf-library >/dev/null 2>&1; then
  echo FORMAL_RELEASE_BLOCK=V2.5.24_ALREADY_EXISTS
  exit 31
fi
if gh api repos/llhzx2018/vf-library/git/ref/tags/v2.5.24 >/dev/null 2>&1; then
  echo FORMAL_TAG_BLOCK=V2.5.24_ALREADY_EXISTS
  exit 32
fi
gh release create v2.5.24 build/formal-a/* \
  --repo llhzx2018/vf-library \
  --target "$PRODUCT_REF" \
  --title 'VF Library V2.5.24' \
  --notes-file build/formal-a/VF_Library_V2.5.24_RELEASE_NOTES.md

gh release view v2.5.24 --repo llhzx2018/vf-library --json databaseId,tagName,isDraft,isPrerelease,publishedAt,targetCommitish >"$RUNNER_TEMP/release2524.json"
jq -e '.tagName=="v2.5.24" and .isDraft==false and .isPrerelease==false' "$RUNNER_TEMP/release2524.json" >/dev/null
mkdir -p "$RUNNER_TEMP/readback2524"
gh release download v2.5.24 --repo llhzx2018/vf-library --pattern "$UPDATE_NAME" --dir "$RUNNER_TEMP/readback2524"
RBYTES=$(stat -c%s "$RUNNER_TEMP/readback2524/$UPDATE_NAME")
RSHA=$(sha256sum "$RUNNER_TEMP/readback2524/$UPDATE_NAME"|awk '{print $1}')
test "$RBYTES" = "$(stat -c%s build/formal-a/$UPDATE_NAME)"
test "$RSHA" = "$(sha256sum build/formal-a/$UPDATE_NAME|awk '{print $1}')"
TAGSHA=$(gh api repos/llhzx2018/vf-library/git/ref/tags/v2.5.24 --jq .object.sha)
test "$TAGSHA" = "$PRODUCT_REF"
echo RELEASE_ID="$(jq -r .databaseId "$RUNNER_TEMP/release2524.json")"
echo PUBLISHED_AT="$(jq -r .publishedAt "$RUNNER_TEMP/release2524.json")"
