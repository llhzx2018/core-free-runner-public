...
def send_keys(session: str, element: str, value: str) -> None:
    journey.session_value(session, 'POST', f'/element/{element}/clear', {})
    journey.session_value(session, 'POST', f'/element/{element}/value', {'text': value, 'value': list(value)})


journey.button_with_text = button_with_text
journey.button_contains = button_contains
journey.send_keys = send_keys
journey.main()
