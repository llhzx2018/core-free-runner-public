...
grep -Fx "define('VF_VERSION', '2.37.1');" candidate/src/app/bootstrap.php >/dev/null
test "$(cat production/VERSION)" = "$SOURCE_VERSION"
jq -e '.production_version=="2.37.0" and .published_release.version=="2.37.0" and .production_release.version=="2.37.0" and .candidate_version=="2.37.1" and .schema_version=="2026082901" and .v2_37_1_release_candidate.schema_change==false and .v2_37_1_release_candidate.migration==null and .current_change.release_authorized_by_owner==true' candidate/VF_PROJECT.json >/dev/null
...
grep -F "'data-original' => 920" candidate/src/app/ResourceCoverCache.php >/dev/null
grep -F "'data-src' => 880" candidate/src/app/ResourceCoverCache.php >/dev/null
grep -F "'data-lazy-src' => 860" candidate/src/app/ResourceCoverCache.php >/dev/null
grep -F 'vf-cover-retry:v2:' candidate/src/assets/workspace.js >/dev/null
printf 'P01_V2371_EXACT_SOURCE=PASS\nRUNTIME_DELTA_4=PASS\nSCHEMA_CHANGE=NO\nMIGRATION=NONE\nOWNER_PRODUCTION_WRITE=NO\n' | tee "$EVID/boundary.txt"
'''

build=run_block(T,'Build deterministic candidate artifacts')
for old,new in [
    ('/tmp/p01-v2360-build.py','/tmp/p01-v2371-build.py'),
    ('/tmp/p01-v2360-first','/tmp/p01-v2371-first'),
    ('P01_V2360','P01_V2371'),('p01-v2360','p01-v2371'),('V2.36.0','V2.37.1'),('v2.36.0','v2.37.1'),
    ('repair-v2.36.0.php','repair-v2.37.1.php'),
...
for old,new in [('p01-v2360','p01-v2371'),('P01_V2360','P01_V2371')]: atomic=atomic.replace(old,new)
atomic=atomic.replace('2.35.3','__SOURCE__').replace('2.36.0','2.37.1').replace('__SOURCE__','2.37.0')
atomic=atomic.replace('P01_V2353_TO_V2371_ATOMIC','P01_V2370_TO_V2371_ATOMIC')

fresh=run_block(T,'Strict fresh candidate runtime')
for old,new in [('p01-v2360','p01-v2371'),('P01_V2360','P01_V2371'),('2.36.0','2.37.1')]: fresh=fresh.replace(old,new)

for name,text in [('01-exact.sh',exact),('02-build.sh',build),('03-atomic.sh',atomic),('04-fresh.sh',fresh)]:
    p=Path('/tmp/p01-v2371-generated')/name
