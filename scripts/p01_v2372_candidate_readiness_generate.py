...
grep -Fx "define('VF_VERSION', '2.37.2');" candidate/src/app/bootstrap.php >/dev/null
test "$(cat production/VERSION)" = "$SOURCE_VERSION"
jq -e '.production_version=="2.37.1" and .published_release.version=="2.37.1" and .production_release.version=="2.37.1" and .candidate_version=="2.37.2" and .schema_version=="2026082901" and .v2_37_2_release_candidate.schema_change==false and .v2_37_2_release_candidate.migration==null and .current_change.release_authorized_by_owner==true' candidate/VF_PROJECT.json >/dev/null
...
printf 'P01_V2372_EXACT_SOURCE=PASS\nRUNTIME_DELTA_4=PASS\nSCHEMA_CHANGE=NO\nMIGRATION=NONE\nOWNER_PRODUCTION_WRITE=NO\n' | tee "$EVID/boundary.txt"
'''

build=run_block(T,'Build deterministic candidate artifacts')
for old,new in [
    ('/tmp/p01-v2360-build.py','/tmp/p01-v2372-build.py'),
    ('/tmp/p01-v2360-first','/tmp/p01-v2372-first'),
    ('P01_V2360','P01_V2372'),('p01-v2360','p01-v2372'),('V2.36.0','V2.37.2'),('v2.36.0','v2.37.2'),
    ('repair-v2.36.0.php','repair-v2.37.2.php'),
...
for old,new in [('p01-v2360','p01-v2372'),('P01_V2360','P01_V2372')]: atomic=atomic.replace(old,new)
atomic=atomic.replace('2.35.3','__SOURCE__').replace('2.36.0','2.37.2').replace('__SOURCE__','2.37.1')
atomic=atomic.replace('P01_V2353_TO_V2372_ATOMIC','P01_V2371_TO_V2372_ATOMIC')

fresh=run_block(T,'Strict fresh candidate runtime')
for old,new in [('p01-v2360','p01-v2372'),('P01_V2360','P01_V2372'),('2.36.0','2.37.2')]: fresh=fresh.replace(old,new)

for name,text in [('01-exact.sh',exact),('02-build.sh',build),('03-atomic.sh',atomic),('04-fresh.sh',fresh)]:
    p=Path('/tmp/p01-v2372-generated')/name
