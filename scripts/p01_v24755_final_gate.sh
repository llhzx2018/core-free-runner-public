...
ART=/tmp/p01-v24755-artifacts
REL=/tmp/p01-rel54
TARGET=2.47.55
SOURCE=2.47.54
SCHEMA=2026090401

python3 scripts/p01_build_v24755_final.py | tee /tmp/p01-v24755-build.json

FULL="$ART/VF-Start-V2.47.55-FULL.zip"
UPDATE="$ART/VF_Start_V2.47.55_UPDATE.zip"

test -f "$FULL"
test -f "$UPDATE"
rm -rf /tmp/p01-v24755-update
unzip -q "$UPDATE" -d /tmp/p01-v24755-update
REPAIR=/tmp/p01-v24755-update/repair-v2.47.55.php
test -f "$REPAIR"
php -l "$REPAIR" >/dev/null
php "$REPAIR" --self-test | tee /tmp/p01-v24755-atomic-selftest.json
grep -Fq '"ok":true' /tmp/p01-v24755-atomic-selftest.json
...
cat >/tmp/p01-v24754-upgrade/sentinel.php <<'PHP'
<?php
...
PHP
php /tmp/p01-v24754-upgrade/sentinel.php
rm /tmp/p01-v24754-upgrade/sentinel.php

cp "$REPAIR" /tmp/p01-v24754-upgrade/repair-v2.47.55.php
php /tmp/p01-v24754-upgrade/repair-v2.47.55.php --verify-source=/tmp/p01-v24754-upgrade | tee /tmp/p01-source-verify.json
grep -Fq '"ok":true' /tmp/p01-source-verify.json

php /tmp/p01-v24754-upgrade/repair-v2.47.55.php --run=/tmp/p01-v24754-upgrade | tee /tmp/p01-atomic-success.json
grep -Fq '"ok":true' /tmp/p01-atomic-success.json
test "$(tr -d '\r\n ' </tmp/p01-v24754-upgrade/VERSION.txt)" = "$TARGET"
(cd /tmp/p01-v24754-upgrade && php cli/verify.php) | grep -Fx 'VERIFY_PASS=YES'
php -r 'require "/tmp/p01-v24754-upgrade/app/bootstrap.php"; $id=(int)trim(file_get_contents("/tmp/p01-sentinel-id.txt")); $s=vf_db()->prepare("SELECT title,is_private FROM links WHERE id=?"); $s->execute([$id]); $r=$s->fetch(PDO::FETCH_ASSOC); if(!$r||$r["title"]!=="Release Sentinel Link"||(int)$r["is_private"]!==1) exit(1); echo "SENTINEL_PRESERVED\n";'
...
PHP
php /tmp/p01-v24754-rollback/sentinel.php
rm /tmp/p01-v24754-rollback/sentinel.php
cp "$REPAIR" /tmp/p01-v24754-rollback/repair-v2.47.55.php
set +e
VF_ATOMIC_TEST_FAIL_AFTER_APPLY=1 php /tmp/p01-v24754-rollback/repair-v2.47.55.php --run=/tmp/p01-v24754-rollback >/tmp/p01-atomic-failure.out 2>/tmp/p01-atomic-failure.err
status=$?
set -e
test "$status" -ne 0
