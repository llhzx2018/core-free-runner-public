...
test "$(git -C candidate rev-parse HEAD^)" = "$SOURCE"
...
grep -Fx "define('VF_VERSION', '2.36.4');" candidate/src/app/bootstrap.php >/dev/null
test "$(cat production/VERSION)" = "$SOURCE_VERSION"
jq -e '.production_version=="2.36.3" and .candidate_version=="2.36.4" and .schema_version=="2026082901" and .v2_36_4_release_candidate.schema_change==false and .v2_36_4_release_candidate.migration==null and .v2_36_4_release_candidate.product_pr==186 and .v2_36_4_release_candidate.product_merge=="0a48be7a99d19fe8485b14b87cc5236682d4e24d" and .published_release.version=="2.36.3" and .published_release.release_source=="718e043e3715ef7b21849bc08634fca89bf92c1f"' candidate/VF_PROJECT.json >/dev/null
...
test "$(wc -l < "$EVID/runtime-delta.txt" | tr -d ' ')" = 3
...
grep -F '.vf-asset-row:not(:has(> .vf-asset-select))' candidate/src/assets/workspace-domain-nav.css >/dev/null
grep -F '.vf-asset-card:not(:has(> .vf-asset-select))' candidate/src/assets/workspace-domain-nav.css >/dev/null
...
printf 'P01_V2364_EXACT_SOURCE=PASS\nP01_V2364_RUNTIME_DELTA_3=PASS\nP01_V2364_SCHEMA_CHANGE=NO\nP01_V2364_MIGRATION=NONE\nANONYMOUS_GRID_STATIC=PASS\nONE_FRONTEND_STATIC=PASS\nSTICKY_STATIC=PASS\nOWNER_PRODUCTION_WRITE=NO\n' | tee "$EVID/boundary.txt"
'''

build=run_block(T,'Build deterministic candidate artifacts')
for old,new in [
    ('/tmp/p01-v2360-build.py','/tmp/p01-v2364-build.py'),
    ('V2.36.0','V2.36.4'),('V2360','V2364'),('v2360','v2364'),
    ('repair-v2.36.0.php','repair-v2.36.4.php'),
]:
...
atomic=atomic.replace('v2360','v2364').replace('V2360','V2364')
atomic=atomic.replace('2.35.3','__P01_SOURCE_VERSION__').replace('2.36.0','2.36.4').replace('__P01_SOURCE_VERSION__','2.36.3')
atomic=atomic.replace('P01_V2353_TO_V2364_ATOMIC','P01_V2363_TO_V2364_ATOMIC')

fresh=run_block(T,'Strict fresh candidate runtime')
fresh=fresh.replace('v2360','v2364').replace('V2360','V2364').replace('2.36.0','2.36.4')

for name,text in [('01-exact.sh',exact),('02-build.sh',build),('03-atomic.sh',atomic),('04-fresh.sh',fresh)]:
    p=Path('/tmp/p01-v2364-generated')/name
