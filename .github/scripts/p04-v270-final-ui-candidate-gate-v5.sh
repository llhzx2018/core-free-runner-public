...
ROOT="${GITHUB_WORKSPACE}/vf-src"; cd "$ROOT"
...
# The V2.5.6 UI contract itself documents that its shell assertions were superseded by V2.5.7.
...
VF_INFRA_E2E_BASE_URL=http://127.0.0.1:18998 VF_INFRA_E2E_ADMIN_CREDENTIAL="$VF_INFRA_E2E_ADMIN_CREDENTIAL" VF_INFRA_E2E_WEB_ROOT="$ROOT/public" VF_V270_TESTED_RUNTIME_COMMIT="$PRODUCT_HEAD" VF_V270_EVIDENCE_DIR="$ROOT/evidence-final-ui" node tests/e2e/v270-final-polish-evidence.mjs | tee evidence-final-ui/e2e.log

test -f public/config.php
mode="$(stat -c '%a' public/config.php)"; case "$mode" in 600|640) ;; *) echo "UNSAFE_CONFIG_MODE=$mode"; exit 71;; esac
...
{"project":"P04 · VF Infra","status":"READY_FOR_ACTUAL_PIXEL_REVIEW","runtime_commit":"$PRODUCT_HEAD","authority_commit":"$AUTHORITY_HEAD","version":"$PRODUCT_VERSION","schema":14,"migration":"NONE","owner_real_use":"PASS","final_ui_polish":"TESTED","functional_regression":"PASS","security_regression":"PASS","desktop":"PASS","mobile":"PASS","actual_pixel_review":"PENDING_MASTER_AGENT_INSPECTION","screenshot_count":$pngs,"production_write":0,"candidate_created":false,"legacy_ui_contracts":"SUPERSEDED_BY_CURRENT_V270_E2E"}
JSON
