...
helper="""    /** @return array<string,mixed> */
    private static function updateContractEvidence():array
    {
        ...
        $single=$has($page,'id=\"installUpdate\"')&&$has($ui,'function startInstall()')&&$has($ui,'update_prepare')&&$has($ui,'update_install');
        $canonical=$has($manager,'public function prepare(): array')&&$has($manager,'vf_assert_disk_space')&&$has($manager,'VfBackupManager')&&$has($manager,'VfAtomicPackage::selfTest()')&&$has($manager,'public function install(string $operationId): array')&&$has($manager,'VfAtomicPackage::run(')&&$has($manager,'journal[')&&$has($manager,"'success'")&&$has($manager,'removeTree(');
        $preflight=$ordered($manager,['vf_assert_disk_space','stagePath(','VfAtomicPackage::run(']);
        $selfTest=$has($manager,'VfAtomicPackage::selfTest()')&&$has($manager,'selfTestJson[')&&$has($manager,'atomicJson[')&&$has($manager,'actual !==');
        $successSafe=$ordered($manager,['atomicJson[','actual !==','journal['])&&$has($manager,"'failed_rolled_back'")&&$has($manager,"'recovery_required'");
