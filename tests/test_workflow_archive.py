...
class WorkflowArchiveTests(unittest.TestCase):
    ...
    def test_manifest_contains_only_public_safe_metadata(self):
        ...
        self.assertEqual(value["schema"], "core-free-runner-workflow-archive/v15")
        self.assertEqual(value["entry_count"], 523)
        self.assertEqual(value["category_counts"]["historical-version"], 398)
        self.assertEqual(value["category_counts"]["late-active"], 86)
        self.assertEqual(value["base_manifest"]["path"], MODULE.V14_MANIFEST_PATH.as_posix())
        self.assertEqual(value["delta"]["git_tree_sha"], MODULE.V15_BATCH_TREE_SHA)
        self.assertEqual(value["delta"]["source_commit"], MODULE.V15_BATCH_SOURCE_COMMIT)
    ...
    def test_v13_p07_batch_is_tree_locked_and_inactive(self):
        root = Path(__file__).resolve().parents[1]
        archived = list((root / MODULE.V13_BATCH).glob("*.yml"))
        self.assertEqual(len(archived), MODULE.V13_BATCH_ENTRY_COUNT)
        self.assertEqual(MODULE._git_tree_sha(root, MODULE.V13_BATCH), MODULE.V13_BATCH_TREE_SHA)
        active = {path.name for path in (root / ".github/workflows").glob("*.yml")}
        self.assertTrue(active.isdisjoint({path.name for path in archived}))

    def test_v14_p07_rc3_batch_is_tree_locked_and_inactive(self):
        root = Path(__file__).resolve().parents[1]
        archived = list((root / MODULE.V14_BATCH).glob("*.yml"))
        self.assertEqual(len(archived), MODULE.V14_BATCH_ENTRY_COUNT)
    ...
    def test_v15_p07_slot2_batch_is_tree_locked_and_inactive(self):
        root = Path(__file__).resolve().parents[1]
        archived = list((root / MODULE.V15_BATCH).glob("*.yml"))
        self.assertEqual(len(archived), MODULE.V15_BATCH_ENTRY_COUNT)
        self.assertEqual(MODULE._git_tree_sha(root, MODULE.V15_BATCH), MODULE.V15_BATCH_TREE_SHA)
        active = {path.name for path in (root / ".github/workflows").glob("*.yml")}
        self.assertTrue(active.isdisjoint({path.name for path in archived}))
