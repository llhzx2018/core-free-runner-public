...
class WorkflowArchiveTests(unittest.TestCase):
    ...
    def test_v16_p07_network_node_old_exact_source_gates_are_inactive(self):
        root = Path(__file__).resolve().parents[1]
        archived = list((root / MODULE.V16_BATCH).glob("*.yml"))
        self.assertEqual(len(archived), MODULE.V16_BATCH_ENTRY_COUNT)
        self.assertEqual(MODULE._git_tree_sha(root, MODULE.V16_BATCH), MODULE.V16_BATCH_TREE_SHA)
        active = {path.name for path in (root / ".github/workflows").glob("*.yml")}
        self.assertTrue(active.isdisjoint({path.name for path in archived}))
        self.assertIn("p07-network-node-package-gate.yml", active)
        self.assertIn("p07-network-node-installer-smoke.yml", active)
