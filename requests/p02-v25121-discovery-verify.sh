...
SITE="$RUNNER_TEMP/p02-v25122-discovery-site"
COOKIE="$RUNNER_TEMP/p02-v25122-discovery-cookie.txt"
BASE="http://127.0.0.1:18291"
PASSWORD="P02-V25122-DISCOVERY-${GITHUB_RUN_ID}!"
...
php -d display_errors=0 -S 127.0.0.1:18291 -t "$SITE" >"$RUNNER_TEMP/p02-v25122-discovery-server.log" 2>&1 &
PID=$!
...
curl -fsS -c "$COOKIE" "$BASE/setup.php" > "$RUNNER_TEMP/p02-v25122-discovery-setup.html"
TOKEN="$(python3 - "$RUNNER_TEMP/p02-v25122-discovery-setup.html" <<'PY'
import html,re,sys
...
jq -e '.ok==true and .site.auth==true and .version=="2.5.121"' <<<"$SESSION" >/dev/null
...
jq -e '
  ...
  .core.from_version=="2.5.121" and
  .core.target_version=="2.5.122" and
  .status.current_version=="2.5.121" and
  .status.latest_version=="2.5.122" and
...
jq -e '.ok==true and .notes.version=="2.5.122"' <<<"$NOTES" >/dev/null

echo P02_V25122_STABLE_DISCOVERY_2_5_121_TO_2_5_122=PASS
