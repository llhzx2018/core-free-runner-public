...
TAG="v2.5.104"
VERSION="2.5.104"
SOURCE_SHA="c91758d59ded8a7b0df385920cdaf76a0a40e5a0"
SOURCE_TREE="3b9e676c3209b59b2d2ae0965f2630f9a748c8f8"
PRODUCT="$PWD/product"
OUT="$PRODUCT/build/formal-release"
REMOTE="$RUNNER_TEMP/p02-v25104-remote-release"
...
rm -rf "$OUT" "$REMOTE"
python3 scripts/build-release.py   --out build/formal-release   --source-version 2.5.81   --source-version 2.5.82   --source-version 2.5.102   --source-version 2.5.103   --source-commit "$SOURCE_SHA"   --source-tree "$SOURCE_TREE"   --source-ref main   --release-summary "Production rail-drag bugfix: visible global navigation and Notebook title rails now support drag-to-resize with click-to-collapse, persistence, and responsive CSS priority correction."
...
gh release create "$TAG"   --repo "$REPO"   --target "$SOURCE_SHA"   --title "P02 · VF Library V2.5.104"   --notes-file "VF_Library_V2.5.104_RELEASE_NOTES.md"   "${ASSETS[@]}"

TAG_SHA="$(gh api "repos/$REPO/git/ref/tags/$TAG" --jq '.object.sha')"
test "$TAG_SHA" = "$SOURCE_SHA"

RELEASE_JSON="$RUNNER_TEMP/p02-v25104-release.json"
gh api "repos/$REPO/releases/tags/$TAG" > "$RELEASE_JSON"
python3 - "$RELEASE_JSON" "$SOURCE_SHA" <<'PY'
import json,sys
x=json.load(open(sys.argv[1]))
assert x["tag_name"]=="v2.5.104"
...
expected=sorted([
 "VF_Library_V2.5.104_ATOMIC.zip",
 "VF_Library_V2.5.104_FULL.zip",
 "VF_Library_V2.5.104_RELEASE_MANIFEST.json",
 "VF_Library_V2.5.104_RELEASE_NOTES.md",
 "VF_Library_V2.5.104_SOURCE.zip",
 "VF_Library_V2.5.104_UPDATE.zip",
 "repair-v2.5.104.php",
 "SHA256SUMS.txt",
])
assert names==expected,(names,expected)
assert all(int(a["size"])>0 for a in x["assets"])
print("P02_V25104_RELEASE_METADATA=PASS")
...
print("P02_V25104_REMOTE_ASSET_READBACK=PASS")
PY

echo "P02_V25104_FORMAL_RELEASE=PASS"
