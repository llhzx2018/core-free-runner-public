...
TAG="v2.5.121"
VERSION="2.5.121"
SOURCE_SHA="df8505cd8e82042c6e9636b2ceb22c742743e14f"
SOURCE_TREE="2c172e957bdcb1440d57907c9bb7592b812ec303"
PRODUCT="$PWD/product"
OUT="$PRODUCT/build/formal-release"
REMOTE="$RUNNER_TEMP/p02-v25121-remote-release"
...
gh release create "$TAG" --repo "$REPO" --target "$SOURCE_SHA" --title "P02 · VF Library V2.5.121" --notes-file "VF_Library_V2.5.121_RELEASE_NOTES.md" "${ASSETS[@]}"

TAG_SHA="$(gh api "repos/$REPO/git/ref/tags/$TAG" --jq '.object.sha')"
test "$TAG_SHA" = "$SOURCE_SHA"
RELEASE_JSON="$RUNNER_TEMP/p02-v25121-release.json"
gh api "repos/$REPO/releases/tags/$TAG" > "$RELEASE_JSON"

python3 - "$RELEASE_JSON" "$SOURCE_SHA" <<'PY'
import json,sys
x=json.load(open(sys.argv[1]))
assert x["tag_name"]=="v2.5.121"
assert x["draft"] is False and x["prerelease"] is False
assert x["target_commitish"]==sys.argv[2]
expected=sorted([
 "VF_Library_V2.5.121_ATOMIC.zip",
 "VF_Library_V2.5.121_FULL.zip",
 "VF_Library_V2.5.121_RELEASE_MANIFEST.json",
 "VF_Library_V2.5.121_RELEASE_NOTES.md",
 "VF_Library_V2.5.121_SOURCE.zip",
 "VF_Library_V2.5.121_UPDATE.zip",
 "repair-v2.5.121.php",
 "SHA256SUMS.txt",
])
assert sorted(a["name"] for a in x["assets"])==expected
assert all(int(a["size"])>0 for a in x["assets"])
print("P02_V25121_RELEASE_METADATA=PASS")
...
print("P02_V25121_REMOTE_ASSET_READBACK=PASS")
PY

echo "P02_V25121_FORMAL_RELEASE=PASS"
