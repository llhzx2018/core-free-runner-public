...
TAG="v2.5.119"
VERSION="2.5.119"
SOURCE_SHA="3ef178367d6a0038b62dd610d7229b146f39fdb0"
SOURCE_TREE="b11ee170ec4b89cba936bb0c2a8230b149497d6d"
PRODUCT="$PWD/product"
OUT="$PRODUCT/build/formal-release"
REMOTE="$RUNNER_TEMP/p02-v25119-remote-release"
...
rm -rf "$OUT" "$REMOTE"
python3 scripts/build-release.py   --out build/formal-release   --source-version 2.5.81   --source-version 2.5.82   --source-version 2.5.102   --source-version 2.5.103   --source-version 2.5.104   --source-version 2.5.105   --source-version 2.5.111   --source-version 2.5.112   --source-version 2.5.113   --source-version 2.5.114   --source-version 2.5.115   --source-version 2.5.116   --source-version 2.5.117   --source-version 2.5.118   --source-commit "$SOURCE_SHA"   --source-tree "$SOURCE_TREE"   --source-ref main   --release-summary "Online Update quiet-state micro closure: normal latest state is unboxed, last-check emphasis is reduced, and spacing is tightened; Settings IA, update behavior and Schema 2401 unchanged."
...
gh release create "$TAG" --repo "$REPO" --target "$SOURCE_SHA" --title "P02 · VF Library V2.5.119" --notes-file "VF_Library_V2.5.119_RELEASE_NOTES.md" "${ASSETS[@]}"

TAG_SHA="$(gh api "repos/$REPO/git/ref/tags/$TAG" --jq '.object.sha')"
test "$TAG_SHA" = "$SOURCE_SHA"
RELEASE_JSON="$RUNNER_TEMP/p02-v25119-release.json"
gh api "repos/$REPO/releases/tags/$TAG" > "$RELEASE_JSON"

python3 - "$RELEASE_JSON" "$SOURCE_SHA" <<'PY'
import json,sys
x=json.load(open(sys.argv[1]))
assert x["tag_name"]=="v2.5.119"
assert x["draft"] is False and x["prerelease"] is False
assert x["target_commitish"]==sys.argv[2]
expected=sorted([
 "VF_Library_V2.5.119_ATOMIC.zip",
 "VF_Library_V2.5.119_FULL.zip",
 "VF_Library_V2.5.119_RELEASE_MANIFEST.json",
 "VF_Library_V2.5.119_RELEASE_NOTES.md",
 "VF_Library_V2.5.119_SOURCE.zip",
 "VF_Library_V2.5.119_UPDATE.zip",
 "repair-v2.5.119.php",
 "SHA256SUMS.txt",
])
assert sorted(a["name"] for a in x["assets"])==expected
assert all(int(a["size"])>0 for a in x["assets"])
print("P02_V25119_RELEASE_METADATA=PASS")
...
print("P02_V25119_REMOTE_ASSET_READBACK=PASS")
PY

echo "P02_V25119_FORMAL_RELEASE=PASS"
