...
def main():
    ...
    notes.write_text(
        f"# VF Library V{version}\n\n"
        "Final Inkstone-class UX/UI convergence release. "
        "The visible shell now uses a single app.css runtime stylesheet; superseded v2536/v2537 CSS files are retired. "
        "The workbench keeps one compact sort/view/density menu plus direct + New, the note rail is continuous rather than card-stacked, and the editor uses a continuous writing plane with docked title, compact properties and right outline. "
        "Medium-width desktop space allocation and authenticated boot transitions are also refined, while the V2.5.39 Settings child-dialog layering fix remains intact. "
        "The private single-admin data model, auth boundary, updater, canonical content model and Schema 2401 remain unchanged.\n\n"
