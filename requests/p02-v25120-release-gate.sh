...
run_upgrade(){
  ...
  local FROM="$2"
  local PORT="$3"
  local SITE="${RUNNER_TEMP}/p02-v25120-from-${FROM}"
  local COOKIE="${RUNNER_TEMP}/p02-v25120-${FROM}-cookie.txt"
  local BASE="http://127.0.0.1:${PORT}"
  local LOG="${RUNNER_TEMP}/p02-v25120-${FROM}-server.log"
...
import json,sys
marker='P02_V25120_UPGRADE_MARKER_'+sys.argv[3].replace('.','_')
json.dump({
  'category_id':int(sys.argv[2]),
  'title':'V2.5.120 Upgrade Fixture '+sys.argv[3],
...
PY
  ...
  cp "$PRODUCT/build/release-preflight/repair-v2.5.120.php" "$SITE/repair-v2.5.120.php"
  curl -fsS -b "$COOKIE" -c "$COOKIE" "$BASE/repair-v2.5.120.php" > "${RUNNER_TEMP}/repair-${FROM}-form.html"
...
PY
  ...
  curl -fsS -b "$COOKIE" -c "$COOKIE" -H "Origin: $BASE"     --data-urlencode "action=upgrade" --data-urlencode "csrf=$REPAIR_CSRF"     "$BASE/repair-v2.5.120.php" > "${RUNNER_TEMP}/repair-${FROM}-result.html"
  grep -q "升级完成" "${RUNNER_TEMP}/repair-${FROM}-result.html"
  test ! -e "$SITE/repair-v2.5.120.php"
...
import json,sys
x=json.load(open(sys.argv[1]))
assert x['ok'] and x['site']['auth'] and x['version']=='2.5.120'
PY
...
marker='P02_V25120_UPGRADE_MARKER_'+sys.argv[2].replace('.','_')
...
PY
  ...
  echo "P02_V25120_DIRECT_UPGRADE_${FROM}=PASS"
}
...
run_upgrade "$ROOT/old115-src" "2.5.115" "18111"
run_upgrade "$ROOT/old116-src" "2.5.116" "18112"
run_upgrade "$ROOT/old117-src" "2.5.117" "18113"
run_upgrade "$ROOT/old118-src" "2.5.118" "18114"
run_upgrade "$ROOT/old119-src" "2.5.119" "18115"
echo P02_V25120_SOURCE_SET_MATRIX=PASS
