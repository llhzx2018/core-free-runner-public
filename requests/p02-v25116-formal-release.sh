...
TAG="v2.5.116"
VERSION="2.5.116"
SOURCE_SHA="fcea97c157a7bb2c864df0ae5fcb99c2c3950536"
SOURCE_TREE="1311d890916cc425626572f1bc7d6f74a54dffbc"
PRODUCT="$PWD/product"
OUT="$PRODUCT/build/formal-release"
REMOTE="$RUNNER_TEMP/p02-v25116-remote-release"
...
rm -rf "$OUT" "$REMOTE"
python3 scripts/build-release.py   --out build/formal-release   --source-version 2.5.81   --source-version 2.5.82   --source-version 2.5.102   --source-version 2.5.103   --source-version 2.5.104   --source-version 2.5.105   --source-version 2.5.111   --source-version 2.5.112   --source-version 2.5.113   --source-version 2.5.114   --source-version 2.5.115   --source-commit "$SOURCE_SHA"   --source-tree "$SOURCE_TREE"   --source-ref main   --release-summary "Inkstone-inspired Settings surface refinement: replace heavy gray matrix panels and strong boxes with warm-white surfaces, subtle dividers, quiet inset controls and restrained cyan accents across all nine Settings pages while preserving layout, behavior and Schema 2401."
...
gh release create "$TAG"   --repo "$REPO"   --target "$SOURCE_SHA"   --title "P02 · VF Library V2.5.116"   --notes-file "VF_Library_V2.5.116_RELEASE_NOTES.md"   "${ASSETS[@]}"

TAG_SHA="$(gh api "repos/$REPO/git/ref/tags/$TAG" --jq '.object.sha')"
test "$TAG_SHA" = "$SOURCE_SHA"

RELEASE_JSON="$RUNNER_TEMP/p02-v25116-release.json"
gh api "repos/$REPO/releases/tags/$TAG" > "$RELEASE_JSON"
python3 - "$RELEASE_JSON" "$SOURCE_SHA" <<'PY'
import json,sys
x=json.load(open(sys.argv[1]))
assert x["tag_name"]=="v2.5.116"
...
expected=sorted([
 "VF_Library_V2.5.116_ATOMIC.zip",
 "VF_Library_V2.5.116_FULL.zip",
 "VF_Library_V2.5.116_RELEASE_MANIFEST.json",
 "VF_Library_V2.5.116_RELEASE_NOTES.md",
 "VF_Library_V2.5.116_SOURCE.zip",
 "VF_Library_V2.5.116_UPDATE.zip",
 "repair-v2.5.116.php",
 "SHA256SUMS.txt",
])
assert names==expected,(names,expected)
assert all(int(a["size"])>0 for a in x["assets"])
print("P02_V25116_RELEASE_METADATA=PASS")
...
print("P02_V25116_REMOTE_ASSET_READBACK=PASS")
PY

echo "P02_V25116_FORMAL_RELEASE=PASS"
