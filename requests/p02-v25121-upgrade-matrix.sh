...
TARGET="2.5.124"
SCHEMA="2401"
SOURCES=(2.5.81 2.5.82 2.5.102 2.5.103 2.5.104 2.5.105 2.5.111 2.5.112 2.5.113 2.5.114 2.5.115 2.5.116 2.5.117 2.5.118 2.5.119 2.5.120 2.5.121 2.5.122)
...
run_one(){
  local FROM="$1"
  local INDEX="$2"
  local SRC="$RUNNER_TEMP/p02-v25124-src-${FROM}"
  local SITE="$RUNNER_TEMP/p02-v25124-site-${FROM}"
  local COOKIE="$RUNNER_TEMP/p02-v25124-cookie-${FROM}.txt"
  local PORT="$((18400 + INDEX))"
  local BASE="http://127.0.0.1:${PORT}"
  local PASS="P02-V25124-UP-${FROM}-${GITHUB_RUN_ID}!"
  local LOG="$RUNNER_TEMP/p02-v25124-${FROM}.log"
...
PY
  ...
  local CAT CID
  CAT="$(curl -fsS -b "$COOKIE" -H 'Content-Type: application/json' -H "X-CSRF-Token: $CSRF"     -d '{"name":"V25124 Direct Upgrade","description":"release matrix fixture","icon":"folder"}'     "$BASE/api.php?action=category_save")"
  CID="$(jq -r .id <<<"$CAT")"
  test "$CID" -gt 0

  local ITEM SAVED IID
  ITEM="$(jq -nc --argjson cid "$CID" --arg f "$FROM" '{category_id:$cid,title:("V25124 Upgrade "+$f),description:"preserve",content:("P02_V25124_MARKER_"+($f|gsub("\\.";"_"))+"\n中文资料\n"+("DATA\n"*80)),content_mode:"article",content_format:"markdown",primary_action:"read",status:"active"}')"
  ...
  python3 - "$RUNNER_TEMP/p02-v25124-${FROM}.png" <<'PY'
import base64,sys
...
PY
  curl -fsS -b "$COOKIE" -H "X-CSRF-Token: $CSRF"     -F "item_id=$IID" -F "attachment=@$RUNNER_TEMP/p02-v25124-${FROM}.png;type=image/png"     "$BASE/api.php?action=attachment_upload" | jq -e '.ok==true and (.attachments|length)>=1' >/dev/null

  cat > "$RUNNER_TEMP/p02-v25124-up-${FROM}.php" <<'PHP'
<?php
$site=$argv[1];
$pkg=$argv[2];
$bytes=(int)$argv[3];
...
PHP
  ...
  php "$RUNNER_TEMP/p02-v25124-up-${FROM}.php" "$SITE" "$PKG" "$BYTES" "$SHA" "$FROM" "$TARGET" "$SOURCE_JSON" "$RUNNER_TEMP/p02-v25124-result-${FROM}.json"
  test "$(cat "$SITE/VERSION.txt")" = "$TARGET"
  jq -e '.backup_locator|length>0' "$RUNNER_TEMP/p02-v25124-result-${FROM}.json" >/dev/null
  ...
  curl -fsS -b "$COOKIE" "$BASE/api.php?action=content_get&id=$IID" |     jq -e --arg f "$FROM" '(.item.is_favorite|tonumber)==1 and (.item.content|contains("P02_V25124_MARKER_"+($f|gsub("\\.";"_"))))' >/dev/null
  ...
  (cd "$SITE" && php cli/verify.php) | jq -e '.ok==true and .version=="2.5.124" and .schema_version==2401 and .integrity=="ok" and .foreign_key_errors==0' >/dev/null
  local DB_FILE
  ...
  echo "P02_V25124_DIRECT_UPGRADE_${FROM}=PASS"
}
...
echo P02_V25124_SOURCE_SET_MATRIX=PASS
