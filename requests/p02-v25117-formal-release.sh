...
TAG="v2.5.117"
VERSION="2.5.117"
SOURCE_SHA="25832773eaa5ee2898cd1005369c8649a808d7bf"
SOURCE_TREE="64366f612d273802b4c32c8a6198cbe2b7eb675d"
PRODUCT="$PWD/product"
OUT="$PRODUCT/build/formal-release"
REMOTE="$RUNNER_TEMP/p02-v25117-remote-release"
...
rm -rf "$OUT" "$REMOTE"
python3 scripts/build-release.py   --out build/formal-release   --source-version 2.5.81   --source-version 2.5.82   --source-version 2.5.102   --source-version 2.5.103   --source-version 2.5.104   --source-version 2.5.105   --source-version 2.5.111   --source-version 2.5.112   --source-version 2.5.113   --source-version 2.5.114   --source-version 2.5.115   --source-version 2.5.116   --source-commit "$SOURCE_SHA"   --source-tree "$SOURCE_TREE"   --source-ref main   --release-summary "Settings final Inkstone closure across all nine Settings pages while preserving layout, behavior and Schema 2401."
...
gh release create "$TAG" --repo "$REPO" --target "$SOURCE_SHA" --title "P02 · VF Library V2.5.117" --notes-file "VF_Library_V2.5.117_RELEASE_NOTES.md" "${ASSETS[@]}"

TAG_SHA="$(gh api "repos/$REPO/git/ref/tags/$TAG" --jq '.object.sha')"
test "$TAG_SHA" = "$SOURCE_SHA"
RELEASE_JSON="$RUNNER_TEMP/p02-v25117-release.json"
gh api "repos/$REPO/releases/tags/$TAG" > "$RELEASE_JSON"

python3 - "$RELEASE_JSON" "$SOURCE_SHA" <<'PY'
import json,sys
x=json.load(open(sys.argv[1]))
assert x["tag_name"]=="v2.5.117"
assert x["draft"] is False and x["prerelease"] is False
assert x["target_commitish"]==sys.argv[2]
expected=sorted([
 "VF_Library_V2.5.117_ATOMIC.zip",
 "VF_Library_V2.5.117_FULL.zip",
 "VF_Library_V2.5.117_RELEASE_MANIFEST.json",
 "VF_Library_V2.5.117_RELEASE_NOTES.md",
 "VF_Library_V2.5.117_SOURCE.zip",
 "VF_Library_V2.5.117_UPDATE.zip",
 "repair-v2.5.117.php",
 "SHA256SUMS.txt",
])
assert sorted(a["name"] for a in x["assets"])==expected
assert all(int(a["size"])>0 for a in x["assets"])
print("P02_V25117_RELEASE_METADATA=PASS")
...
print("P02_V25117_REMOTE_ASSET_READBACK=PASS")
PY

echo "P02_V25117_FORMAL_RELEASE=PASS"
