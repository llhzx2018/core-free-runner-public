...
run_upgrade(){
  ...
  local FROM="$2"
  local PORT="$3"
  local SITE="${RUNNER_TEMP}/p02-v25118-from-${FROM}"
  local COOKIE="${RUNNER_TEMP}/p02-v25118-${FROM}-cookie.txt"
  local BASE="http://127.0.0.1:${PORT}"
  local LOG="${RUNNER_TEMP}/p02-v25118-${FROM}-server.log"
...
import json,sys
marker='P02_V25118_UPGRADE_MARKER_'+sys.argv[3].replace('.','_')
json.dump({
  'category_id':int(sys.argv[2]),
  'title':'V2.5.118 Upgrade Fixture '+sys.argv[3],
...
PY
  ...
  cp "$PRODUCT/build/release-preflight/repair-v2.5.118.php" "$SITE/repair-v2.5.118.php"
  curl -fsS -b "$COOKIE" -c "$COOKIE" "$BASE/repair-v2.5.118.php" > "${RUNNER_TEMP}/repair-${FROM}-form.html"
...
PY
  ...
  curl -fsS -b "$COOKIE" -c "$COOKIE" -H "Origin: $BASE"     --data-urlencode "action=upgrade" --data-urlencode "csrf=$REPAIR_CSRF"     "$BASE/repair-v2.5.118.php" > "${RUNNER_TEMP}/repair-${FROM}-result.html"
  grep -q "升级完成" "${RUNNER_TEMP}/repair-${FROM}-result.html"
  test ! -e "$SITE/repair-v2.5.118.php"
  test "$(cat "$SITE/VERSION.txt")" = "2.5.118"
...
import json,sys
x=json.load(open(sys.argv[1]))
assert x['ok'] and x['site']['auth'] and x['version']=='2.5.118'
PY
...
marker='P02_V25118_UPGRADE_MARKER_'+sys.argv[2].replace('.','_')
...
PY
  ...
  echo "P02_V25118_DIRECT_UPGRADE_${FROM}=PASS"
}
...
echo P02_V25118_SOURCE_SET_MATRIX=PASS
