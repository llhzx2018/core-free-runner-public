...
TAG="v2.5.114"
VERSION="2.5.114"
SOURCE_SHA="47322276f6c0405943692585d51fd80973ee60a6"
SOURCE_TREE="156e4a5bb9b63db93f4af195d6fa5a1c8476fbb8"
PRODUCT="$PWD/product"
OUT="$PRODUCT/build/formal-release"
REMOTE="$RUNNER_TEMP/p02-v25114-remote-release"
...
rm -rf "$OUT" "$REMOTE"
python3 scripts/build-release.py   --out build/formal-release   --source-version 2.5.81   --source-version 2.5.82   --source-version 2.5.102   --source-version 2.5.103   --source-version 2.5.104   --source-version 2.5.105   --source-version 2.5.111   --source-version 2.5.112   --source-version 2.5.113   --source-commit "$SOURCE_SHA"   --source-tree "$SOURCE_TREE"   --source-ref main   --release-summary "Settings mature surface polish: unify all nine Settings pages around mature typography, hierarchy, secondary-copy contrast, control rhythm and responsive density while preserving IA, behavior and Schema 2401."
...
gh release create "$TAG"   --repo "$REPO"   --target "$SOURCE_SHA"   --title "P02 · VF Library V2.5.114"   --notes-file "VF_Library_V2.5.114_RELEASE_NOTES.md"   "${ASSETS[@]}"

TAG_SHA="$(gh api "repos/$REPO/git/ref/tags/$TAG" --jq '.object.sha')"
test "$TAG_SHA" = "$SOURCE_SHA"

RELEASE_JSON="$RUNNER_TEMP/p02-v25114-release.json"
gh api "repos/$REPO/releases/tags/$TAG" > "$RELEASE_JSON"
python3 - "$RELEASE_JSON" "$SOURCE_SHA" <<'PY'
import json,sys
x=json.load(open(sys.argv[1]))
assert x["tag_name"]=="v2.5.114"
...
expected=sorted([
 "VF_Library_V2.5.114_ATOMIC.zip",
 "VF_Library_V2.5.114_FULL.zip",
 "VF_Library_V2.5.114_RELEASE_MANIFEST.json",
 "VF_Library_V2.5.114_RELEASE_NOTES.md",
 "VF_Library_V2.5.114_SOURCE.zip",
 "VF_Library_V2.5.114_UPDATE.zip",
 "repair-v2.5.114.php",
 "SHA256SUMS.txt",
])
assert names==expected,(names,expected)
assert all(int(a["size"])>0 for a in x["assets"])
print("P02_V25114_RELEASE_METADATA=PASS")
...
print("P02_V25114_REMOTE_ASSET_READBACK=PASS")
PY

echo "P02_V25114_FORMAL_RELEASE=PASS"
