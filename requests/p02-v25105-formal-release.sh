...
TAG="v2.5.105"
VERSION="2.5.105"
SOURCE_SHA="c103134ba9e23f83d68c9676cdf892fb2629e8f8"
SOURCE_TREE="2d514d03327cd873625255ffac8defda86c1144d"
PRODUCT="$PWD/product"
OUT="$PRODUCT/build/formal-release"
REMOTE="$RUNNER_TEMP/p02-v25105-remote-release"
...
rm -rf "$OUT" "$REMOTE"
python3 scripts/build-release.py   --out build/formal-release   --source-version 2.5.81   --source-version 2.5.82   --source-version 2.5.102   --source-version 2.5.103   --source-version 2.5.104   --source-commit "$SOURCE_SHA"   --source-tree "$SOURCE_TREE"   --source-ref main   --release-summary "Inkstone Final Alignment: viewport-aware three-pane geometry protection, quieter divider rails, compact unified Settings rhythm, and lower-noise workspace chrome."
...
gh release create "$TAG"   --repo "$REPO"   --target "$SOURCE_SHA"   --title "P02 · VF Library V2.5.105"   --notes-file "VF_Library_V2.5.105_RELEASE_NOTES.md"   "${ASSETS[@]}"

TAG_SHA="$(gh api "repos/$REPO/git/ref/tags/$TAG" --jq '.object.sha')"
test "$TAG_SHA" = "$SOURCE_SHA"

RELEASE_JSON="$RUNNER_TEMP/p02-v25105-release.json"
gh api "repos/$REPO/releases/tags/$TAG" > "$RELEASE_JSON"
python3 - "$RELEASE_JSON" "$SOURCE_SHA" <<'PY'
import json,sys
x=json.load(open(sys.argv[1]))
assert x["tag_name"]=="v2.5.105"
...
expected=sorted([
 "VF_Library_V2.5.105_ATOMIC.zip",
 "VF_Library_V2.5.105_FULL.zip",
 "VF_Library_V2.5.105_RELEASE_MANIFEST.json",
 "VF_Library_V2.5.105_RELEASE_NOTES.md",
 "VF_Library_V2.5.105_SOURCE.zip",
 "VF_Library_V2.5.105_UPDATE.zip",
 "repair-v2.5.105.php",
 "SHA256SUMS.txt",
])
assert names==expected,(names,expected)
assert all(int(a["size"])>0 for a in x["assets"])
print("P02_V25105_RELEASE_METADATA=PASS")
...
print("P02_V25105_REMOTE_ASSET_READBACK=PASS")
PY

echo "P02_V25105_FORMAL_RELEASE=PASS"
