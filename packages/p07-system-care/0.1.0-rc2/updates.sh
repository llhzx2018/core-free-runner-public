...
apply_security() {
  ...
  if [[ ${#packages[@]} -eq 0 ]]; then ok '当前缓存列表中没有识别到安全更新。'; show_state; return 0; fi
  printf '将仅升级以下安全更新包：\n'
  printf '  %s\n' "${packages[@]}"
...
case "${1:-menu}" in
  ...
  menu)
    while true; do
      ...
      case "$choice" in
        1) if confirm_numeric '刷新 APT 更新列表？'; then refresh; show_state; else warn '已取消。'; fi; pause_menu ;;
