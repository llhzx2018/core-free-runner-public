...
RC2 boundary:
  ...
  - menu rendering is lightweight and never runs APT simulation.
...
show_header() {
  ...
  local summary health updates disk failed
  summary="$(bash "$SCRIPT_DIR/status.sh" quick 2>/dev/null || true)"
  health="$(value_from_summary "$summary" P07_SYSTEM_CARE_STATUS)"
  updates="$(value_from_summary "$summary" P07_SYSTEM_CARE_SECURITY_UPDATES)"
  disk="$(value_from_summary "$summary" P07_SYSTEM_CARE_ROOT_DISK_PCT)"
  failed="$(value_from_summary "$summary" P07_SYSTEM_CARE_FAILED_UNITS)"

  [[ "$health" == HEALTHY ]] && printf '状态   %b正常%b\n' "$C_GREEN" "$C_RESET" || printf '状态   %b需关注%b\n' "$C_YELLOW" "$C_RESET"
  if [[ "$updates" =~ ^[0-9]+$ ]]; then
    printf '更新   %s 个安全更新\n' "$updates"
  else
    printf '更新   未检查\n'
  fi
  [[ "$disk" =~ ^[0-9]+$ ]] && printf '磁盘   %s%%\n' "$disk" || printf '磁盘   UNKNOWN\n'
  [[ "$failed" =~ ^[0-9]+$ ]] && printf '服务   %s 个异常\n' "$failed" || printf '服务   UNKNOWN\n'
  say
}
