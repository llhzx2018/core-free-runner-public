...
RC11 candidate boundary:
  ...
  - top status uses plain-language server / website safety / automatic-check states.
  - "服务器需检查" means open the audit to see evidence; it does not imply an automatic repair exists.
  ...
  - WordPress enrollment first reuses CloudPanel inventory facts and never treats generic PHP sites as WordPress.
