...
attention=0
issues=()
need_updates=0 need_cleanup=0 need_memory=0 need_services=0 need_security=0 need_reboot=0
...
if [[ "$root_login" == yes ]]; then issues+=("SSH 允许 root 直接登录"); attention=$((attention+1)); need_security=1; fi
if [[ "$password_auth" == yes ]]; then issues+=("SSH 允许密码认证"); attention=$((attention+1)); need_security=1; fi

health=HEALTHY
[[ "$attention" -gt 0 ]] && health=ATTENTION
write_summary_cache "$health" "$disk" "$inode" "$failed" "$reboot_cache" "$cp"
...
if [[ "$attention" -eq 0 ]]; then
  ok '结果       正常 · 当前未发现需要立即查看的底层问题'
else
  warn "结果       需检查 · ${attention} 项"
  ...
  (( need_security == 1 )) && say '  - SSH 配置风险：返回后选 6. SSH / 安全检查'
  (( need_reboot == 1 )) && say '  - 需要重启：请在你自己的维护窗口手动安排；P07 不会自动重启'
  (( need_security == 1 )) && say '  - SSH / 防火墙页目前只检查，不存在隐藏的自动修复入口。'
fi
say
