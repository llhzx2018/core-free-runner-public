...
preflight_enable() {
  ...
  case "$WP_DISCOVERY_STATUS" in
    READY)
      say "CloudPanel   已识别 ${WP_CLOUDPANEL_SITES} 个网站"
      say "WordPress    已识别 ${WP_SITES} 个"
      say
    ...
    NO_WORDPRESS)
      ...
      say 'WordPress    0 个'
      say
      warn '结果         当前没有识别到可开启“网站入侵留证”的 WordPress 网站。'
      say '说明         P07 不会把普通 PHP 网站误当成 WordPress，也不会创建空的自动任务。'
      say '下一步       不用重复按开启；以后这台服务器有 WordPress 后再开启即可。'
      return 0
      ;;
    UNSAFE_LAYOUT)
...
menu_not_enabled() {
  ...
  while true; do
    ...
    say '网站安全   未开启'
    say '自动检查   未开启'
    say '下一步     选择 1；P07 会先判断这台服务器有没有 WordPress'
    say
    say '1. 开启网站安全保护'
    say '0. 返回'
    say
