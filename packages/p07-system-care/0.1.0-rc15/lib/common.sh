...
cache_file_for() {
  local name="$1" uid="${EUID:-0}"
  printf '/tmp/p07-system-care-%s-%s.cache\n' "$name" "$uid"
}

write_kv_cache() {
  local name="$1"; shift
  local uid="${EUID:-0}" file="/tmp/p07-system-care-${name}-${EUID:-0}.cache" tmp item
  tmp="${file}.tmp.$$"
  umask 077
...
cache_get() {
  local name="$1" key="$2" outvar="$3"
  local uid="${EUID:-0}" file="/tmp/p07-system-care-${name}-${EUID:-0}.cache"
  local k v result='UNKNOWN'
  if [[ -r "$file" ]]; then
...
write_summary_cache() {
  local health="${1:-UNKNOWN}" disk="${2:-UNKNOWN}" inode="${3:-UNKNOWN}" failed="${4:-UNKNOWN}" reboot="${5:-UNKNOWN}" cp="${6:-UNKNOWN}" advisories="${7:-UNKNOWN}"
