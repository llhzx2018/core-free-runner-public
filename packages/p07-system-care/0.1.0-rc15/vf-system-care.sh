...
Usage:
  ...
  vf-system-care.sh resources [menu|preview|matrix|json|plan|apply|backups|rollback] [--mode conservative|balanced|performance]
...
RC15 candidate boundary:
  ...
  - operational server health and security advice are separate cached projections.
  - root/password SSH findings are advice by default, not an operational-health failure.
  - expensive checks run only after the user selects an explicit action.
  - resource profiles combine hardware baselines with runtime measurements and remain read-only recommendations.
  - resource profile preview never writes PHP/MySQL/Swap/systemd and never restarts services.
  - RC15 uses a calibration registry: Production-verified profiles may be Safe Apply eligible; Candidate/Preview-only profiles stay blocked.
  - 1C/2GB Balanced is Production-verified; 2C/4GB Balanced is Candidate until real calibration; Safe Apply never restarts MySQL or auto-mutates Swap / unused PHP services.
  ...
  - entering website evidence performs one bounded CloudPanel/WordPress classification before offering enrollment.
  - generic PHP sites are never treated as WordPress just to make enrollment pass.
