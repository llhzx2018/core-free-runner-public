...
if (( risk_count == 0 )); then
  ok '安全建议     当前没有 P07 明确标记的 SSH 建议'
else
  warn "安全建议     ${risk_count} 项 · 不代表服务器故障"
  [[ "$root" == yes ]] && say '             - Root 可直接 SSH 登录'
  [[ "$pass" == yes ]] && say '             - SSH 允许密码认证'
  say '说明         这些配置可能是你当前的正常登录方式，因此 P07 只提醒，不自动修改。'
...
say
say "${C_GRAY}此页只检查，不修改 SSH、防火墙、Fail2ban 或 CloudPanel 规则。安全建议和服务器运行故障是两回事。${C_RESET}"
