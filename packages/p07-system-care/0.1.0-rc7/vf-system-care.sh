...
Usage:
  ...
  vf-system-care.sh evidence [menu|status|enable|scan|report|rebaseline|disable]
  vf-system-care.sh --version
  vf-system-care.sh --help

RC7 candidate boundary:
...
show_header() {
  ...
  local health updates disk failed evidence
  ...
  case "$evidence" in
    NORMAL) printf '留证   %b正常%b\n' "$C_GREEN" "$C_RESET" ;;
    ATTENTION) printf '留证   %b需关注%b\n' "$C_YELLOW" "$C_RESET" ;;
    FAILED) printf '留证   %b检查失败%b\n' "$C_RED" "$C_RESET" ;;
    NOT_ENABLED) printf '留证   未启用\n' ;;
    *) printf '留证   未检查\n' ;;
  esac
  say
}
