...
summary_value() {
  local key="$1"
  bash "$SCRIPT_DIR/status.sh" 2>/dev/null | awk -F= -v k="$key" '$1==k {print $2; exit}'
}
...
show_header() {
  ...
  health="$(summary_value P07_SYSTEM_CARE_STATUS)"; updates="$(summary_value P07_SYSTEM_CARE_SECURITY_UPDATES)"
  disk="$(summary_value P07_SYSTEM_CARE_ROOT_DISK_PCT)"; failed="$(summary_value P07_SYSTEM_CARE_FAILED_UNITS)"
