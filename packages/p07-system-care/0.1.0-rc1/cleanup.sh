...
apt_cache_kb() {
  ...
  du -sk /var/cache/apt/archives 2>/dev/null | awk '{print $1+0}'
}
...
case "${1:-menu}" in
  ...
  menu)
    while true; do
      ...
      say "${C_BOLD}P07 · 磁盘 / 日志清理${C_RESET}"; say; scan; say
      say '1. 清理 APT 缓存'
      say '2. 清理旧 systemd 日志（保留 14 天）'
      say '0. 返回'; say
      printf '请选择 [0-2]：'; read -r choice || exit 0
      case "$choice" in
        1) clean_apt; pause_menu ;;
        2) clean_journal; pause_menu ;;
        0) exit 0 ;;
        *) warn '无效选择。'; sleep 1 ;;
      esac
    done
