...
require_apt() { [[ "$(pkg_manager)" == apt ]] || { fail '当前 RC1 仅支持 Debian/Ubuntu APT 更新。'; return 65; }; }
...
apply_security() {
  ...
  if [[ ${#packages[@]} -eq 0 ]]; then ok '当前缓存列表中没有识别到安全更新。'; return 0; fi
  printf '将仅升级以下安全更新包：\n'
  printf '  %s\n' "${packages[@]}"
...
case "${1:-menu}" in
  ...
  menu)
    while true; do
      ...
      say "${C_BOLD}P07 · 系统更新${C_RESET}"; say; show_state; say
      say '1. 刷新更新列表'
      say '2. 安装安全更新'
      say '3. 安装全部更新'
      say '0. 返回'; say
      ...
      case "$choice" in
        1) if confirm_numeric '刷新 APT 更新列表？'; then refresh; else warn '已取消。'; fi; pause_menu ;;
        2) apply_security; pause_menu ;;
        3) apply_all; pause_menu ;;
        0) exit 0 ;;
        *) warn '无效选择。'; sleep 1 ;;
      esac
    done
