...
if [[ "$mode" == quick ]]; then
  ...
  health="$(cached_summary_value STATUS)"
  disk="$(cached_summary_value DISK)"
  inode="$(cached_summary_value INODE)"
  failed="$(cached_summary_value FAILED)"
  reboot="$(cached_summary_value REBOOT)"
  cp="$(cached_summary_value CLOUDPANEL)"
else
