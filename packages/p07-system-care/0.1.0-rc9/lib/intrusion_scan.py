...
def _cloudpanel_sites() -> tuple[bool, list[tuple[str, str]]]:
    """Return (authoritative, domain/user rows) from the CloudPanel DB when supported."""
    path = cloudpanel_db_path()
    if not path.is_file():
        return False, []
    try:
    ...
    try:
        ...
        if not {"domain_name", "user"}.issubset(columns):
            return False, []
        rows = [
            (str(domain), str(user))
            for domain, user in conn.execute('SELECT domain_name, user FROM "site" ORDER BY domain_name')
            if domain not in (None, "") and user not in (None, "")
        ]
        return True, rows
    except sqlite3.Error:
        return False, []
...
def _nginx_document_roots() -> dict[str, list[Path]]:
    """Best-effort map of server_name -> literal root paths from CloudPanel nginx vhosts."""
    ...
    for conf in sorted(directory.glob("*.conf")):
        ...
        for match in re.finditer(r"(?m)^\s*server_name\s+([^;]+);", text):
            names.update(name for name in re.split(r"\s+", match.group(1).strip()) if name and name != "_")
        literal_roots: list[Path] = []
        for match in re.finditer(r"(?m)^\s*root\s+([^;]+);", text):
            value = match.group(1).strip().strip('"\'')
            if not value or "$" in value or not value.startswith("/"):
                continue
            literal_roots.append(Path(value))
        for name in names:
            bucket = roots.setdefault(name, [])
...
def _validated_candidate(root: Path, home_resolved: Path) -> Path | None:
    marker = root / "wp-config.php"
    try:
    ...
    try:
        ...
        raise UnsupportedWordPressLayoutError(
            f"WordPress root escapes the protected CloudPanel home tree: {root}"
        ) from exc
    require_regular_marker(resolved)
    return resolved


def _discover_cloudpanel(home_resolved: Path) -> tuple[bool, list[Path]]:
    authoritative, rows = _cloudpanel_sites()
    if not authoritative:
        return False, []
    vhost_roots = _nginx_document_roots()
    found: dict[str, Path] = {}
    for domain, user in rows:
        candidates: list[Path] = [home_resolved / user / "htdocs" / domain]
        for root in vhost_roots.get(domain, []):
            if root not in candidates:
                candidates.append(root)
        for candidate in candidates:
            resolved = _validated_candidate(candidate, home_resolved)
            if resolved is not None:
                found[str(resolved)] = resolved
    return True, [found[key] for key in sorted(found)]


def _discover_fallback(home: Path, home_resolved: Path) -> list[Path]:
    seen: dict[str, Path] = {}
    for pattern in ("*/htdocs/*/wp-config.php", "*/htdocs/*/*/wp-config.php"):
        for marker in home.glob(pattern):
            resolved = _validated_candidate(marker.parent, home_resolved)
            if resolved is not None:
...
def discover(explicit: list[str]) -> list[Path]:
    ...
    cloudpanel_authoritative, cloudpanel_roots = _discover_cloudpanel(home_resolved)
    if cloudpanel_authoritative:
        return cloudpanel_roots
    return _discover_fallback(home, home_resolved)
