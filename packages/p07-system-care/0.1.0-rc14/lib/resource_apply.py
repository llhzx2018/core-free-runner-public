#!/usr/bin/env python3
"""P07 Resource Safe Apply RC14.

Auto-apply is intentionally restricted to the production-calibrated
1 vCPU / 2 GB / Balanced / CloudPanel lane. Other profiles remain preview-only.
"""
...
def build_plan(
    ...
    *,
    ...
    reasons: List[str] = []
    if recommendation.get("evidence", {}).get("production_calibration") != "1C_2G_BALANCED_REFERENCE":
        reasons.append("PROFILE_NOT_PRODUCTION_CALIBRATED")
    if recommendation.get("mode") != "balanced":
        reasons.append("MODE_NOT_BALANCED")
    if int(snapshot.get("cpu_count") or 0) != 1:
        reasons.append("CPU_NOT_1C")
    if recommendation.get("hardware", {}).get("band") != "2G":
        reasons.append("RAM_NOT_2G_BAND")
