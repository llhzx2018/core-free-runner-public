...
run_upgrade_simulation() {
  local outvar="$1" _simulation rc
  set +e
  _simulation="$(apt-get -s -o Debug::NoLocking=true upgrade 2>&1)"
  rc=$?
  set -e
  printf -v "$outvar" '%s' "$_simulation"
  return "$rc"
}
...
security_install_preflight() {
  ...
  local packages=("$@") _simulation rc protected=()
  [[ ${#packages[@]} -gt 0 ]] || return 0

  set +e
  _simulation="$(apt-get -s -o Debug::NoLocking=true install --only-upgrade "${packages[@]}" 2>&1)"
  rc=$?
  set -e
  printf -v "$outvar" '%s' "$_simulation"
  [[ $rc -eq 0 ]] || { fail '安全更新模拟安装失败，已阻止真实安装。'; return 93; }

  if simulation_has_downgrade_or_removal "$_simulation"; then
    warn '安全更新计划出现降级或移除包，已阻止真实安装。'
    return 94
  fi

  if cloudpanel_present; then
    mapfile -t protected < <(simulation_protected_packages "$_simulation")
    if [[ ${#protected[@]} -gt 0 ]]; then
