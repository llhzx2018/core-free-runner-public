...
# Ownership is recorded immediately, but it remains explicitly PENDING until
# live service + UDP health pass. Failed health can never be shown as healthy.
write_state PENDING
...
if ! wait_for_udp_listener "$port" 20; then
  ...
  fail "已标记为安装异常，可从菜单选择安装 / 修复节点再次自动修复。诊断日志：$health_log"
  exit 13
fi
write_state HEALTHY
ok "服务与 UDP 检查通过"
