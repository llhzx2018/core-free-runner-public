#!/usr/bin/env bash
set -Eeuo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"
# shellcheck source=lib/common.sh
source "$SCRIPT_DIR/lib/common.sh"

quiet=0
[[ "${1:-}" == "--quiet" ]] && quiet=1

load_state
cli="$(v2ray_cli || true)"
managed=0
[[ -r "$VF_NODE_STATE_FILE" ]] && managed=1
active=0
service_active && active=1
listener=0
if [[ -n "${PORT:-}" ]] && valid_port "$PORT" && port_in_use_udp "$PORT"; then
  listener=1
fi
healthy=0
if ((managed && active && listener)); then
  healthy=1
fi

if ((quiet)); then
  ((healthy)) && exit 0
  exit 1
fi

section "VF Node · 节点状态"
say "${C_GRAY}──────────────────────────────────────────────────────────────${C_RESET}"
printf '%-18s %b\n' "模块版本" "${C_CYAN}${VF_NODE_VERSION}${C_RESET}"

if ((managed)); then
  if ((healthy)); then
    printf '%-18s %b\n' "管理状态" "${C_GREEN}● P07 已接管 · 健康${C_RESET}"
  else
    printf '%-18s %b\n' "管理状态" "${C_RED}● P07 已接管 · 安装异常${C_RESET}"
  fi
  printf '%-18s %b\n' "安装标记" "${C_GRAY}${INSTALL_STATUS:-LEGACY_STATE}${C_RESET}"
  printf '%-18s %b\n' "Profile" "${C_GREEN}${PROFILE:-UNKNOWN}${C_RESET}"
  printf '%-18s %b\n' "V2Ray Core" "${C_CYAN}${CORE_VERSION:-UNKNOWN}${C_RESET}"
  printf '%-18s %b\n' "Core Asset" "${C_GRAY}${CORE_ASSET:-UNKNOWN}${C_RESET}"
  printf '%-18s %b\n' "UDP 端口" "${C_YELLOW}${PORT:-UNKNOWN}${C_RESET}"
  printf '%-18s %b\n' "上游提交" "${C_GRAY}${SOURCE_COMMIT:-UNKNOWN}${C_RESET}"
  printf '%-18s %b\n' "安装时间" "${C_GRAY}${INSTALLED_AT:-UNKNOWN}${C_RESET}"
else
  if [[ -n "$cli" ]]; then
    printf '%-18s %b\n' "管理状态" "${C_YELLOW}● 外部 V2Ray · P07 不接管${C_RESET}"
  else
    printf '%-18s %b\n' "管理状态" "${C_GRAY}● 未安装${C_RESET}"
  fi
fi

if [[ -n "$cli" ]]; then
  printf '%-18s %b\n' "管理命令" "${C_CYAN}${cli}${C_RESET}"
else
  printf '%-18s %b\n' "管理命令" "${C_RED}✗ 未找到${C_RESET}"
fi

if ((active)); then
  printf '%-18s %b\n' "服务" "${C_GREEN}✓ active${C_RESET}"
else
  printf '%-18s %b\n' "服务" "${C_RED}✗ inactive / unknown${C_RESET}"
fi

if [[ -n "${PORT:-}" ]]; then
  if ((listener)); then
    printf '%-18s %b\n' "UDP 监听" "${C_GREEN}✓ ${PORT}/UDP · LISTEN${C_RESET}"
  else
    printf '%-18s %b\n' "UDP 监听" "${C_RED}✗ ${PORT}/UDP · NOT LISTENING${C_RESET}"
  fi
fi

say "${C_GRAY}──────────────────────────────────────────────────────────────${C_RESET}"
say
if ((healthy)); then
  ok "节点基础健康检查 PASS"
  exit 0
fi
if ((managed)); then
  fail "节点已由 P07 管理，但当前没有通过健康检查。RC8 可自动尝试修复。"
  exit 1
fi
if [[ -n "$cli" ]]; then
  warn "发现已有 V2Ray，但当前不是 P07 管理安装。不会自动修改。"
  exit 10
fi
fail "节点基础健康检查未通过"
exit 1
