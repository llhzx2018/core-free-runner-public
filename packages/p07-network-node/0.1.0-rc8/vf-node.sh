...
show_live_summary() {
  ...
  if [[ -r "$VF_NODE_STATE_FILE" ]]; then
    if node_healthy; then
      say "状态   ${C_GREEN}● 已安装 · 运行正常${C_RESET}"
    else
      say "状态   ${C_RED}● 安装异常 · 可自动修复${C_RESET}"
    fi
...
show_menu() {
  ...
  say "${C_GRAY}──────────────────────────────────────────────────────────────${C_RESET}"
  say
  say "  ${C_GREEN}1. [安装]${C_RESET} 安装 / 自动修复稳定节点"
  say "     ${C_GRAY}自动安装 VMess + mKCP + dtls${C_RESET}"
  say
  say "  ${C_CYAN}2. [检查]${C_RESET} 查看节点状态"
  say "     ${C_GRAY}服务 / UDP / Core / 安装信息${C_RESET}"
  say
  say "  ${C_MAGENTA}3. [分享]${C_RESET} 显示分享链接"
  say "     ${C_GRAY}仅健康节点显示 · 包含节点凭据${C_RESET}"
  say
  say "  ${C_BLUE}4. [备份]${C_RESET} 备份节点配置"
  say "     ${C_GRAY}PRIVATE 备份 + SHA256${C_RESET}"
  say
  say "  ${C_YELLOW}5. [实验]${C_RESET} 高速实验 Profile"
  say "     ${C_GRAY}尚未开放 · 不影响稳定节点${C_RESET}"
  say
  say "  ${C_RED}6. [危险]${C_RESET} 卸载节点"
  say "     ${C_GRAY}数字确认 + 自动备份 · 不碰其他服务${C_RESET}"
  say
  say "  ${C_GRAY}0. [退出]${C_RESET} 退出"
  say
}
...
mark_state_healthy() {
  [[ -s "$VF_NODE_STATE_FILE" ]] || return 0
  if grep -q '^INSTALL_STATUS=' "$VF_NODE_STATE_FILE"; then
    sed -i 's/^INSTALL_STATUS=.*/INSTALL_STATUS=HEALTHY/' "$VF_NODE_STATE_FILE"
  else
    printf 'INSTALL_STATUS=HEALTHY\n' >> "$VF_NODE_STATE_FILE"
  fi
  chmod 0600 "$VF_NODE_STATE_FILE"
}

repair_managed_install() {
  info "检测到节点未通过健康检查，正在自动修复..."
  if restart_managed_node 20; then
    mark_state_healthy
    ok "节点自动修复完成，服务与 UDP 已恢复"
    return 0
  fi
  fail "自动修复未成功。不会覆盖配置、UUID 或端口。"
...
install_from_menu() {
  if [[ -r "$VF_NODE_STATE_FILE" ]]; then
    if node_healthy; then
      warn "节点已经安装并运行正常，无需重装。"
    else
      repair_managed_install || true
    fi
    pause_if_tty
    return 0
  fi
...
menu_loop() {
  while true; do
    ...
    case "$choice" in
      ...
      5)
        say
        warn "高速实验 Profile 仍处于设计/验证阶段。稳定节点不会被修改。"
        pause_if_tty
        ;;
...
用法：
  ...
  vf-node install [args]     安装稳定节点
  vf-node status             查看节点状态
  vf-node check              静默健康检查
  vf-node url                仅健康时显示分享链接
  vf-node share              同 url
