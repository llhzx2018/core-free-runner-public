...
load_state() {
  ...
  CORE_VERSION=''; CORE_ASSET=''; CORE_SHA256=''; INSTALL_STATUS=''
...
wait_for_udp_listener() {
  local port="$1" seconds="${2:-20}" i
  valid_port "$port" || return 1
  for ((i=1; i<=seconds; i++)); do
    if service_active && port_in_use_udp "$port"; then
      return 0
    fi
    sleep 1
  done
...
node_healthy() {
  load_state
  [[ -r "$VF_NODE_STATE_FILE" ]] || return 1
  [[ -n "${PORT:-}" ]] && valid_port "$PORT" || return 1
  service_active && port_in_use_udp "$PORT"
}

restart_managed_node() {
  load_state
  [[ -r "$VF_NODE_STATE_FILE" ]] || return 1
  [[ -n "${PORT:-}" ]] && valid_port "$PORT" || return 1
  command -v systemctl >/dev/null 2>&1 || return 1
  systemctl restart v2ray.service >/dev/null 2>&1 || true
  wait_for_udp_listener "$PORT" "${1:-20}"
}
