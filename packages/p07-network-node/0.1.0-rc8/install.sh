...
# RC8 ownership is recorded immediately, but it is explicitly PENDING until
# live service + UDP health pass. This prevents a failed install being shown as healthy.
write_state PENDING

info "启动并检查节点..."
systemctl restart v2ray.service >/dev/null 2>&1 || true
if ! wait_for_udp_listener "$port" 20; then
  write_state FAILED
  health_log="$(write_health_failure_log)"
  fail "节点没有在 20 秒内进入可用状态。"
  fail "已标记为安装异常，可由 RC8 自动修复。诊断日志：$health_log"
  exit 13
fi
write_state HEALTHY
ok "服务与 UDP 检查通过"
